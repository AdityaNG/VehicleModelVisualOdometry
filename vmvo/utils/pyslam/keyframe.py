...
class KeyFrameGraph(object):
    ...
    def get_connected_keyframes(self):
        with self._lock_connections:
            return list(
                self.connected_keyframes_weights.keys()
            )  # returns a copy
    ...
    def get_covisible_keyframes(self):
        with self._lock_connections:
            return list(
                self.ordered_keyframes_weights.keys()
            )  # returns a copy
    ...
    def get_covisible_by_weight(self, weight):
        with self._lock_connections:
            return [
                kf
                for kf, w in self.ordered_keyframes_weights.items()
                if w > weight
            ]
...
class KeyFrame(Frame, KeyFrameGraph):
    def __init__(self, frame, img=None):
        ...
        self.kpsn = (
            frame.kpsn
        )  # [n]ormalized keypoint coordinates     [Nx2] (Kinv * [kp,1])
        self.octaves = (
            frame.octaves
        )  # keypoint octaves                      [Nx1]
        ...
        if hasattr(frame, "_kd"):
            ...
            Printer.orange(
                "KeyFrame %d computing kdtree for input frame %d"
                % (self.id, frame.id)
            )
        ...
        self.outliers = np.full(
            self.kpsu.shape[0], False, dtype=bool
    ...
    def update_connections(self):
        ...
        assert len(points) > 0
        viewing_keyframes = [
            kf for p in points for kf in p.keyframes() if kf.kid != self.kid
        ]  # exclude this keyframe
        ...
        with self._lock_connections:
            if (
                w_max
                >= Parameters.kMinNumOfCovisiblePointsForCreatingConnection
            ):

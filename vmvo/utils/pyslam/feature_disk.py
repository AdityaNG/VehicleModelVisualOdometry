...
# convert matrix of pts into list of keypoints
def convert_pts_to_keypoints_with_translation(
    pts, scores, size, deltax, deltay
):
    assert len(pts) == len(scores)
    kps = []
    if pts is not None:
...
class DiskFeature2D:
    def __init__(
        ...
        self.model_base_path = (
            config.cfg.root_folder + "/thirdparty/disk/depth-save.pth"
        )
        ...
        with self.lock:
            ...
            if self.use_crop:
                self.kps = convert_pts_to_keypoints(
                    keypoints, scores, self.keypoint_size
                )
            else:
    ...
    def compute(
        ...
        with self.lock:
            if self.frame is not frame:
                Printer.orange(
                    "WARNING: DISK is recomputing both kps and des on last input frame",
                    frame.shape,
                )
                self.detectAndCompute(frame)
            return self.kps, self.des

...
def groundtruth_factory(settings):
    ...
    if type == "kitti":
        return KittiGroundTruth(
            path, name, associations, GroundTruthType.KITTI
        )
    if type == "tum":
    ...
    if type == "video" or type == "folder":
        ...
        return SimpleGroundTruth(
            path, name, associations, GroundTruthType.SIMPLE
        )
    else:
        print("not using groundtruth")
        print(
            "if you are using main_vo.py, your estimated trajectory will not make sense!"
        )
        return GroundTruth(
            path, name, associations=None, type=GroundTruthType.NONE
        )
...
class GroundTruth(object):
    def __init__(
        self, path, name, associations=None, type=GroundTruthType.NONE
    ):
        self.path = path
        self.name = name
        self.type = type
...
class SimpleGroundTruth(GroundTruth):
    def __init__(
        self, path, name, associations=None, type=GroundTruthType.KITTI
    ):
...
class TumGroundTruth(GroundTruth):
    def __init__(
        self, path, name, associations=None, type=GroundTruthType.TUM
    ):
        super().__init__(path, name, associations, type)
        ...
        if self.file_associations is not None:
            with open(self.file_associations) as f:
                ...
                self.associations = [
                    line.strip().split() for line in self.associations
                ]
    ...
    def associate(first_list, second_list, offset=0, max_difference=0.02):
        ...
        potential_matches = [
            (
                abs(float(a[0]) - (float(b[0]) + offset)),
                ia,
                ib,

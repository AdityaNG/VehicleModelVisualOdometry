...
kMaxIdDistBetweenIntializingFrames = (
    5  # N.B.: worse performances with values smaller than 5!
)

kFeatureMatchRatioTestInitializer = (
    Parameters.kFeatureMatchRatioTestInitializer
)
...
class Initializer(object):
    def __init__(self):
        ...
        self.idx_f_ref = (
            0  # index of the reference frame in self.frames buffer
        )
        self.f_ref = None
    ...
    def estimatePose(self, kpn_ref, kpn_cur):
        ...
        _, R, t, mask = cv2.recoverPose(
            E, kpn_cur, kpn_ref, focal=1, pp=(0.0, 0.0)
        )
        return poseRt(
            R, t.T
    ...
    def initialize(self, f_cur, img_cur):
        if (
            self.num_failures
            > kNumOfFailuresAfterWichNumMinTriangulatedPointsIsHalved
        ):
            self.num_min_triangulated_points = (
                0.5 * Parameters.kInitializerNumMinTriangulatedPoints
            )
            self.num_failures = 0
            Printer.orange(
        ...
        idxs_cur, idxs_ref = match_frames(
            f_cur, f_ref, kFeatureMatchRatioTestInitializer
        )
        ...
        if new_pts_count > self.num_min_triangulated_points:
            err = map.optimize(
                verbose=False, rounds=20, use_robust_kernel=True
            )

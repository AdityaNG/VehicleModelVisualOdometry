...
class ContextDescFeature2D:
    ...
    def __init__(
        ...
        self.model_base_path = (
            config.cfg.root_folder + "/thirdparty/contextdesc/"
        )
        ...
        self.reg_model_path = (
            self.model_base_path + "pretrained/retrieval_model"
        )
        ...
        if self.model_type == "pb":
            ...
            reg_model_path = os.path.join(
                self.reg_model_path, "model.ckpt-550000"
            )
            loc_model_path = os.path.join(
                self.loc_model_path, "model.ckpt-400000"
            )
            aug_model_path = os.path.join(
                self.loc_model_path, "model.ckpt-400000"
            )
        else:
    ...
    def extract_local_features(self, gray_list):
        ...
        for _, val in enumerate(gray_list):
            (
                ...
                cv_kpts,
                sift_desc,
            ) = self.loc_model.run_test_data(val)
            raw_kpts = [
                np.array((i.pt[0], i.pt[1], i.size, i.angle, i.response))
                for i in cv_kpts
            ]
            raw_kpts = np.stack(raw_kpts, axis=0)
            loc_info = np.concatenate(
                (raw_kpts, normalized_xy, loc_feat, kpt_mb), axis=-1
            )
            cv_kpts_list.append(cv_kpts)
            loc_info_list.append(loc_info)
            sift_feat_list.append(sift_desc)
            loc_feat_list.append(
                loc_feat / np.linalg.norm(loc_feat, axis=-1, keepdims=True)
            )
        # model.close()
    ...
    def extract_augmented_features(self, reg_feat_list, loc_info_list):
        ...
        for idx, _ in enumerate(reg_feat_list):
            aug_feat, _ = self.aug_model.run_test_data(
                [reg_feat_list[idx], loc_info_list[idx]]
            )
    ...
    def compute_kps_des(self, frame):
        with self.lock:
            ...
            # extract augmented features.
            aug_feat_list = self.extract_augmented_features(
                reg_feat_list, loc_info_list
            )

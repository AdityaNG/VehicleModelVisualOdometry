...
def draw_feature_matches_horizontally(
    img1, img2, kps1, kps2, kps1_sizes=None, kps2_sizes=None
):
    ...
    for i, pts in enumerate(zip(kps1, kps2)):
        ...
        cv2.line(
            img3, (a, b), (c + w1, d), color, 1
        )  # join corrisponding points
        cv2.circle(img3, (a, b), 2, color, -1)
        cv2.circle(
            img3, (a, b), color=(0, 255, 0), radius=int(size1), thickness=1
...
def draw_feature_matches(
    img1, img2, kps1, kps2, kps1_sizes=None, kps2_sizes=None, horizontal=True
):
    if horizontal:
        return draw_feature_matches_horizontally(
            img1, img2, kps1, kps2, kps1_sizes, kps2_sizes
        )
    else:
...
def draw_random_rects(img, N=100):
    ...
    for i in range(N):
        ...
        cv2.rectangle(
            img, (pt1x, pt1y), (pt2x, pt2y), color, thickness, lineType
        )

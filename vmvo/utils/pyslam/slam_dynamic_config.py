...
class SLAMDynamicConfig(object):
    ...
    def update_descriptor_stat(self, f_ref, f_cur, idxs_ref, idxs_cur):
        if len(idxs_cur) > 0:
            ...
            sigma_mad, _ = descriptor_sigma_mad(
                des_cur,
                des_ref,
                descriptor_distances=Frame.descriptor_distances,
            )
            ...
            if self.descriptor_distance_sigma is not None:
                self.descriptor_distance_sigma = (
                    self.descriptor_distance_alpha
                    * self.descriptor_distance_sigma
    ...
    def update_reproj_err_map_stat(self, value):
        ...
        self.reproj_err_frame_map_sigma = max(
            1.0, self.reproj_err_frame_map_sigma
        )

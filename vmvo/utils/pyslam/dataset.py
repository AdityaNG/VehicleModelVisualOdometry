...
class Dataset(object):
    def __init__(
        self, path, name, fps=None, associations=None, type=DatasetType.NONE
    ):
        self.path = path
        self.name = name
        self.type = type
    ...
    def getImageColor(self, frame_id):
        try:
            ...
            Printer.red(
                "Cannot open dataset: ", self.name, ", path: ", self.path
            )
            return img
...
class LiveDataset(Dataset):
    ...
    def getImage(self, frame_id):
        ...
        self._timestamp = (
            time.time()
        )  # rough timestamp if nothing else is available
        self._next_timestamp = self._timestamp + self.Ts
        if ret is False:
...
class FolderDataset(Dataset):
    def __init__(
        self, path, name, fps=None, associations=None, type=DatasetType.VIDEO
    ):
...
class TumDataset(Dataset):
    ...
    def getImage(self, frame_id):
        ...
        if frame_id < self.max_frame_id:
            file = (
                self.base_path + self.associations[frame_id].strip().split()[1]
            )
            img = cv2.imread(file)
            self.is_ok = img is not None
    ...
    def getDepth(self, frame_id):
        ...
        if frame_id < self.max_frame_id:
            file = (
                self.base_path + self.associations[frame_id].strip().split()[3]
            )
            img = cv2.imread(file)
            self.is_ok = img is not None
            self._timestamp = float(

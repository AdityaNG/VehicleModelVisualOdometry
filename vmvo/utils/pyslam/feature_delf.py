...
delf_base_path = (
    config.cfg.root_folder
    + "/thirdparty/tensorflow_models/research/delf/delf/python/"
)
delf_config_file = delf_base_path + "examples/delf_config_example.pbtxt"
delf_model_path = (
    delf_base_path + "examples/parameters/delf_gld_20190411/model/"
)
delf_mean_path = (
    delf_base_path + "examples/parameters/delf_gld_20190411/pca/mean.datum"
)
delf_projection_matrix_path = (
    delf_base_path
    + "examples/parameters/delf_gld_20190411/pca/pca_proj_mat.datum"
)
...
def MakeExtractor(sess, config, import_scope=None):
    ...
    input_image = sess.graph.get_tensor_by_name(
        "%sinput_image:0" % import_scope_prefix
    )
...
def convert_pts_to_keypoints(pts, scores, sizes):
    ...
    if pts is not None:
        ...
        kps = [
            cv2.KeyPoint(p[0], p[1], _size=sizes[i], _response=scores[i])
...
class DelfFeature2D:
    def __init__(
        self, num_features=1000, score_threshold=100, do_tf_logging=False
    ):
        ...
        self.delf_config.delf_local_config.pca_parameters.mean_path = (
            delf_mean_path
        )
        ...
        try:
            ...
            self.scale_factor = np.sqrt(
                2
            )  # according to default config and the paper
    ...
    def compute(
        ...
        with self.lock:
            if self.frame is not frame:
                Printer.orange(
                    "WARNING: DELF is recomputing both kps and des on last input frame",
                    frame.shape,
                )
                self.detectAndCompute(frame)
            return self.kps, self.des

...
from .utils_geom import add_ones, normalize_vector, normalize_vector2, poseRt
...
class MapPointBase(object):
    ...
    def __init__(self, id=None):
        ...
        self._observations = (
            dict()
        )  # keyframe observations (used by mapping methods)
        ...
        self.num_times_visible = (
            1  # number of times the point is visible in the camera
        )
        ...
        self.last_frame_id_seen = (
            -1
        )  # last frame id in which this point was seen
    ...
    def observations_string(self):
        obs = sorted(
            [
                (kf.id, kidx, kf.get_point_match(kidx) != None)
    ...
    def frame_views_string(self):
        obs = sorted(
            [
                (f.id, idx, f.get_point_match(idx) != None)
    ...
    def add_frame_view(self, frame, idx):
        ...
        with self._lock_features:
            ...
            ):  # do not allow a point to be matched to diffent keypoints
...
class MapPoint(MapPointBase):
    global_lock = (
        RLock()
    )  # shared global lock for blocking point position update
    ...
    def __init__(self, position, color, keyframe=None, idxf=None, id=None):
        ...
        if keyframe is not None:
            ...
            self._min_distance = (
                self._max_distance
                / Frame.feature_manager.scale_factors[
                    Frame.feature_manager.num_levels - 1
                ]
            )
    ...
    def replace_with(self, p):
        ...
        for (
            kf,
            kidx,
        ) in observations:  # we have kf.get_point_match(kidx) = self
        ...
        with self._lock_features:
            with self._lock_pos:
                ...
                if (
                    ...
                    > self.num_observations_on_last_update_normals
                    or force
    ...
    def update_best_descriptor(self, force=False):
        ...
        with self._lock_features:
            ...
            if (
                self._num_observations
                > self.num_observations_on_last_update_des
                or force
            ):  # implicit if self._num_observations > 1
                self.num_observations_on_last_update_des = (
                    self._num_observations
        ...
        if skip:
            return
        descriptors = [
            kf.des[idx] for kf, idx in observations if not kf.is_bad
        ]
        N = len(descriptors)
        if N > 2:
            ...
            median_distances = [
                np.median(
                    Frame.descriptor_distances(descriptors[i], descriptors)
                )
                for i in range(N)
            ]
            with self._lock_features:
    ...
    def predict_detection_level(self, dist):
        ...
        level = math.ceil(
            math.log(ratio) / Frame.feature_manager.log_scale_factor
        )
        if level < 0:
            level = 0
...
def predict_detection_levels(points, dists):
    ...
    levels = np.ceil(
        np.log(ratios) / Frame.feature_manager.log_scale_factor
    ).astype(np.intp)

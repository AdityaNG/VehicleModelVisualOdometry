...
def convert_pts_to_keypoints(pts, scores, size=1):
    ...
    if pts is not None:
        ...
        if is_opencv_version_greater_equal(4, 5, 3):
            kps = [
                cv2.KeyPoint(p[0], p[1], size=size, response=scores[i])
                for i, p in enumerate(pts)
            ]
        else:
            kps = [
                cv2.KeyPoint(p[0], p[1], _size=size, _response=scores[i])
                for i, p in enumerate(pts)
            ]
    return kps
...
class D2NetFeature2D:
    def __init__(
        ...
        self.models_path = (
            self.model_base_path + "models/d2_ots.pth"
        )  # best performances obtained with 'd2_ots.pth'
        ...
        # Creating CNN model
        self.model = D2Net(
            model_file=self.models_path, use_relu=use_relu, use_cuda=do_cuda
        )
        if self.do_cuda:
        ...
        with self.lock:
            ...
            if max(resized_image.shape) > self.max_edge:
                resized_image = scipy.misc.imresize(
                    resized_image, self.max_edge / max(resized_image.shape)
                ).astype("float")
            ...
            input_image = preprocess_image(
                resized_image, preprocessing=self.preprocessing
            )
            with torch.no_grad():
                if self.multiscale:
                    self.pts, scores, descriptors = process_multiscale(
                        torch.tensor(
                            input_image[np.newaxis, :, :, :].astype(
                                np.float32
                            ),
                            device=self.device,
                        ),
            ...
            self.kps = convert_pts_to_keypoints(
                self.pts, scores, self.keypoint_size
            )
    ...
    def detectAndCompute(self, frame, mask=None):  # mask is a fake input
        with self.lock:
            ...
            if kVerbose:
                print(
                    "detector: D2NET, descriptor: D2NET, #features: ",
                    len(self.kps),
                    ", frame res: ",
                    frame.shape[0:2],
                )
    ...
    def compute(
        ...
        with self.lock:
            if self.frame is not frame:
                Printer.orange(
                    "WARNING: D2NET is recomputing both kps and des on last input frame",
                    frame.shape,
                )
                self.detectAndCompute(frame)
            return self.kps, self.des

...
kTrackingWaitForLocalMappingToGetIdle = (
    Parameters.kTrackingWaitForLocalMappingToGetIdle
)
kTrackingWaitForLocalMappingSleepTime = (
    Parameters.kTrackingWaitForLocalMappingSleepTime
)
...
kRansacThresholdNormalized = (
    0.0003  # 0.0003 # metric threshold used for normalized image coordinates
)
kRansacProb = 0.999
...
kUseMotionModel = (
    Parameters.kUseMotionModel or Parameters.kUseSearchFrameByProjection
)
kUseSearchFrameByProjection = (
    Parameters.kUseSearchFrameByProjection
    and not Parameters.kUseEssentialMatrixFitting
)
...
class Tracking(object):
    def __init__(self, system):
        ...
        self.t0_gt = (
            None  # history of ground truth translations (if available)
        )
        self.traj3d_est = (
            []
    ...
    def estimate_pose_by_fitting_ess_mat(
        self, f_ref, f_cur, idxs_ref, idxs_cur
    ):
    ...
    def pose_optimization(self, f_cur, name=""):
        ...
        ) = optimizer_g2o.pose_optimization(f_cur, verbose=False)
    ...
    def track_previous_frame(self, f_ref, f_cur):
        ...
        use_search_frame_by_projection = (
            self.motion_model.is_ok
            and kUseSearchFrameByProjection
            and kUseMotionModel
        )
        ...
        if use_search_frame_by_projection:
            ...
            print(
                "# matched map points in prev frame: %d "
            ...
            if (
                self.num_matched_kps
                < Parameters.kMinNumMatchedFeaturesSearchFrameByProjection
            ):
            ...
            if (
                ...
                if kUseDynamicDesDistanceTh:
                    self.descriptor_distance_sigma = (
                        self.dyn_config.update_descriptor_stat(
                ...
                if (
                    not self.pose_is_ok
                    or self.num_matched_map_points
                    < kNumMinInliersPoseOptimizationTrackFrame
                ):
                    Printer.red(
        ...
        if (
            not use_search_frame_by_projection
            or is_search_frame_by_projection_failure
        ):
    ...
    def track_local_map(self, f_cur):
        ...
        if kDebugDrawMatches and True:
            ...
            cv2.imshow(
                "tracking local map - matched trails", img_matched_trails
            )
            cv2.waitKey(1)
        ...
        print(
            "     # num_matched_map_points: %d" % (self.num_matched_map_points)
        )
        if (
            not self.pose_is_ok
            or self.num_matched_map_points
            < kNumMinInliersPoseOptimizationTrackLocalMap
        ):
            Printer.red(
    ...
    def need_new_keyframe(self, f_cur):
        ...
        cond3 = (
            num_f_cur_tracked_points
            < num_kf_ref_tracked_points * Parameters.kThNewKfRefRatio
    ...
    def track(self, img, frame_id, timestamp=None):
        ...
        if self.state == SlamState.NOT_INITIALIZED:
            ...
            if intializer_is_ok:
                ...
                Printer.green(
                    "map: initialized %d new points" % (new_pts_count)
                )
        ...
        with self.map.update_lock:
            ...
            if kUseDynamicDesDistanceTh:
                ...
                self.local_mapping.descriptor_distance_sigma = (
                    self.descriptor_distance_sigma
                )
    ...
    def wait_for_local_mapping(self):
        if kTrackingWaitForLocalMappingToGetIdle:
            ...
            if (
                not self.local_mapping.is_idle()

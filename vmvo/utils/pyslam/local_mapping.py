...
from search_points import search_and_fuse, search_frame_for_triangulation
...
if kLocalMappingOnSeparateThread:
    ...
    def print(*args, **kwargs):
        return __builtin__.print(
            *args, **kwargs, file=logging_file, flush=True
        )
...
class LocalMapping(object):
    def __init__(self, map):
        ...
        self.timer_verbose = (
            kTimerVerbose  # set this to True if you want to print timings
        )
        self.timer_triangulation = TimerFps(
            "Triangulation", is_verbose=self.timer_verbose
        )
        self.timer_pts_culling = TimerFps(
            "Culling points", is_verbose=self.timer_verbose
        )
        self.timer_pts_fusion = TimerFps(
            "Fusing points", is_verbose=self.timer_verbose
        )
        self.time_local_opt = TimerFps(
            "Local optimization", is_verbose=self.timer_verbose
        )
        self.time_large_opt = TimerFps(
            "Large window optimization", is_verbose=self.timer_verbose
        )
    ...
    def local_BA(self):
        ...
        err = self.map.locally_optimize(
            kf_ref=self.kf_cur, abort_flag=self.opt_abort_flag
        )
        ...
        Printer.purple(
            "KF(%d) #points: %d " % (self.kf_cur.id, num_kf_ref_tracked_points)
        )
    ...
    def large_window_BA(self):
        ...
        Printer.blue(
            "large window optimization error^2: %f, KF id: %d"
            % (err, self.kf_cur.kid)
        )
    ...
    def cull_map_points(self):
        ...
        for p in self.recently_added_points:
            ...
            ) >= 2 and p.num_observations <= th_num_observations:
                self.map.remove_point(p)
                remove_set.add(p)
            elif (
                current_kid - p.first_kid
    ...
    def cull_keyframes(self):
        ...
        for kf in self.kf_cur.get_covisible_keyframes():
            ...
            kf_num_redundant_observations = (
                0  # num redundant observations for kf
            )
            for i, p in enumerate(kf.get_points()):
                if p is not None and not p.is_bad:
                    kf_num_points += 1
                    if p.num_observations > th_num_observations:
                        scale_level = kf.octaves[
                            i
                        ]  # scale level of observation in kf
                        p_num_observations = 0
                        for kf_j, idx in p.observations():
                            if kf_j is kf:
                                continue
                            assert not kf_j.is_bad
                            scale_level_i = kf_j.octaves[
                                idx
                            ]  # scale level of observation in kfi
                            if (
            ...
            if (
                (
                    kf_num_redundant_observations
                    > Parameters.kKeyframeCullingRedundantObsRatio
                    * kf_num_points
                )
                and (kf_num_points > Parameters.kKeyframeCullingMinNumPoints)
                and (
                    kf.timestamp - kf.parent.timestamp
        ...
        if not Parameters.kLocalMappingParallelKpsMatching:
            ...
            for kf in local_keyframes:
                if kf is self.kf_cur or kf.is_bad:
                    continue
                idxs1, idxs2 = Frame.feature_matcher.match(
                    self.kf_cur.des, kf.des
                )
    ...
    def create_new_map_points(self):
        ...
        for i, kf in enumerate(local_keyframes):
            ...
            if len(idxs_cur) > 0:
                ...
                print(
                    "# added map points: %d for KFs (%d, %d)"
    ...
    def fuse_map_points(self):
        ...
        for kf in local_keyframes:
            ...
            print(
                "# fused map points: %d for KFs (%d, %d)"
                % (num_fused_pts, self.kf_cur.id, kf.id)
            )
        ...
        good_local_points = [
            ...
            if (p is not None and not p.is_bad)
        ]  # all good points in local frames
        good_local_points = np.array(
            list(set(good_local_points))
        )  # be sure to get only one instance per point
        num_fused_pts = search_and_fuse(
            good_local_points,
            self.kf_cur,
            max_reproj_distance=Parameters.kMaxReprojectionDistanceFuse,
        ...
        print(
            "# fused map points: %d for local map into KF %d"
            % (num_fused_pts, self.kf_cur.id)
        )
        total_fused_pts += num_fused_pts

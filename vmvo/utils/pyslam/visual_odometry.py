...
class VisualOdometry(object):
    def __init__(self, cam, groundtruth, feature_tracker):
        ...
        self.timer_verbose = (
            False  # set this to True if you want to print timings
        )
    ...
    def getAbsoluteScale(self, frame_id):
        if self.groundtruth is not None and kUseGroundTruthScale:
            (
                ...
                self.trueZ,
                scale,
            ) = self.groundtruth.getPoseAndAbsoluteScale(frame_id)
            return scale
        else:
            self.trueX = 0
            self.trueY = 0
    ...
    def processFrame(self, frame_id):
        ...
        R, t = self.estimatePose(
            self.track_result.kps_ref_matched,
            self.track_result.kps_cur_matched,
        )
        ...
        if self.stage == VoStage.GOT_FIRST_IMAGE:
            if reinit:
                ...
                for i, pts in enumerate(
                    ...
                    drawAll = (
                        False  # set this to true if you want to draw outliers
                    )
    ...
    def updateHistory(self):
        if (self.init_history is True) and (self.trueX is not None):
            ...
            self.t0_gt = np.array(
                [self.trueX, self.trueY, self.trueZ]
            )  # starting translation
        ...
        if (self.t0_est is not None) and (self.t0_gt is not None):
            ...
            pg = [
                self.trueX - self.t0_gt[0],
                self.trueY - self.t0_gt[1],
                self.trueZ - self.t0_gt[2],
            ]  # the groudtruth traj starts at 0

...
class KeyNetDescFeature2D:
    def __init__(
        ...
        scale_factor_levels=np.sqrt(
            2
        ),  # The scale factor between the pyramid levels.
        do_cuda=True,
        do_tf_logging=False,
    ):
    ...
    def load_model(self):
        ...
        with tf.name_scope("inputs"):
            ...
            self.kpts_coord = tf.placeholder(
                dtype=tf.float32, shape=(None, 2), name="kpts_coord"
            )
            self.kpts_batch = tf.placeholder(
                dtype=tf.int32, shape=(None,), name="kpts_batch"
            )
            self.kpts_scale = tf.placeholder(
                dtype=tf.float32, name="kpts_scale"
            )
        ...
        if os.listdir(self.keynet_config.checkpoint_det_dir):
            (
                init_assign_op_det,
                init_feed_dict_det,
            ) = tf_contrib.framework.assign_from_checkpoint(
                tf.train.latest_checkpoint(
                    self.keynet_config.checkpoint_det_dir
                ),
                detect_var,
            )
        ...
        self.levels = (
            self.keynet_config.pyramid_levels
            + self.keynet_config.upsampled_levels
            + 1
        )
        # print('levels: ', [i for i in range(self.levels)])
        for idx_level in range(self.levels):
            tmp += factor_points ** (
                -1 * (idx_level - self.keynet_config.upsampled_levels)
            )
            point_level.append(
    ...
    def extract_keynet_features(self, image):
        ...
        for j, resized in enumerate(pyramid):
            ...
            score_maps[
                "map_" + str(j + 1 + self.keynet_config.upsampled_levels)
            ] = im_scores[0, :, :, 0]
        ...
        if self.keynet_config.upsampled_levels:
            for j in range(self.keynet_config.upsampled_levels):
                ...
                im = np.reshape(
                    up_image, (1, up_image.shape[0], up_image.shape[1], 1)
                )
        ...
        for idx_level in range(self.levels):
            ...
            if idx_level > 0:
                res_points = int(
                    np.asarray(
                        [self.point_level[a] for a in range(0, idx_level + 1)]
                    ).sum()
                    - len(im_pts)
                )
            ...
            im_scores = rep_tools.apply_nms(
                score_maps["map_" + str(idx_level + 1)],
                self.keynet_config.nms_size,
            )
            ...
            im_pts_levels_tmp = (
                np.ones(len(im_pts), dtype=np.int32) * idx_level
            )
            im_pts_levels = np.concatenate(
                (im_pts_levels, im_pts_levels_tmp), axis=0
            ).astype(np.int32)
        ...
        sorted_idxs = (
            -1 * im_pts[:, 3]
        ).argsort()  # sort points with their scores
        ...
        for idx_desc_batch in range(int(len(im_pts) / 250 + 1)):
            points_batch = im_pts[
                idx_desc_batch * 250 : (idx_desc_batch + 1) * 250
            ]
            ...
            feed_dict = {
                ...
                self.kpts_scale: self.keynet_config.scale_factor
                * points_batch[:, 2],
            ...
            patch_batch = self.session.run(
                self.input_patches, feed_dict=feed_dict
            )
            patch_batch = np.reshape(
                patch_batch, (patch_batch.shape[0], 1, 32, 32)
            )
    ...
    def compute_kps_des(self, im):
        with self.lock:
            ...
            im_pts, descriptors, im_pts_levels = self.extract_keynet_features(
                im
            )

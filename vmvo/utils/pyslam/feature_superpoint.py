...
class SuperPointOptions:
    def __init__(self, do_cuda=True):
        ...
        self.weights_path = (
            config.cfg.root_folder + "/thirdparty/superpoint/superpoint_v1.pth"
        )
        self.nms_dist = 4
...
def convert_superpts_to_keypoints(pts, size=1):
    ...
    if pts is not None:
        ...
        if is_opencv_version_greater_equal(4, 5, 3):
            kps = [
                cv2.KeyPoint(p[0], p[1], size=size, response=p[2]) for p in pts
            ]
        else:
            kps = [
                cv2.KeyPoint(p[0], p[1], _size=size, _response=p[2])
                for p in pts
            ]
    return kps
    ...
    def detectAndCompute(self, frame, mask=None):  # mask is a fake input
        with self.lock:
            ...
            self.kps = convert_superpts_to_keypoints(
                self.pts.T, size=self.keypoint_size
            )
            if kVerbose:
                print(
                    "detector: SUPERPOINT, #features: ",
                    len(self.kps),
                    ", frame res: ",
                    frame.shape[0:2],
                )

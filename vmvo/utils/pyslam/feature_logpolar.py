...
class HardNet(nn.Module):
    ...
    def forward(self, img, theta=None, imgIDs=None):
        if (
            theta is None
        ):  # suppose patches are directly given (as e.g. for external test data)
            patches = img
    ...
    def compute_des(self, img, kps):
        ...
        for i, p in enumerate(pts):
            _p = (
                ...
                * np.array(
                    [
                        (p[0] + padLeft) / (cfg.TEST.PAD_TO),
                        (p[1] + padUp) / (cfg.TEST.PAD_TO),
                    ]
                )
                - 1
            )

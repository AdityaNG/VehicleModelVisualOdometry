...
class MotionModelBase(object):
    ...
    def current_pose(self):
        ...
        """
        return (
            g2o.Isometry3d(self.orientation, self.position),
            self.covariance,
        )

    def predict_pose(
        self, timestamp, prev_position=None, prev_orientation=None
    ):
        return None

    def update_pose(
        self, timestamp, new_position, new_orientation, new_covariance=None
    ):
        return None
...
class MotionModel(MotionModelBase):
    ...
    def update_pose(
        ...
        if self.initialized:
            ...
            self.delta_orientation = (
                new_orientation * self.orientation.inverse()
            )
    ...
    def apply_correction(self, correction):  # corr: g2o.Isometry3d or matrix44
        ...
        # correction= Tcw_corrected * Tcw_uncorrected.inverse()  (transform from camera_uncorrected to camera_corrected)
        self.delta_orientation = (
            correction.orientation() * self.delta_orientation
        )
...
class MotionModelDamping(MotionModelBase):
    ...
    def predict_pose(
        ...
        delta_angle = g2o.AngleAxis(
            self.v_angular_angle * dt * self.damp, self.v_angular_axis
        )
        delta_orientation = g2o.Quaternion(delta_angle)

        orientation = delta_orientation * self.orientation
        position = self.position + delta_orientation * (
            self.v_linear * dt * self.damp
        )

...
class L2Net(nn.Module):
    ...
    def forward(self, input):
        ...
        return nn.LocalResponseNorm(256, 1 * 256, 0.5, 0.5)(x_features).view(
            input.size(0), -1
        )
...
class L2NetFeature2D:
    def __init__(self, do_cuda=True):
        ...
        self.model_weights_path = (
            self.model_base_path + "l2net_ported_weights_lib+.pth"
        )
    ...
    def compute_des_batches(self, patches):
        ...
        for i in range(0, len(patches), self.batch_size):
            data_a = patches[i : i + self.batch_size, :, :, :].astype(
                np.float32
            )

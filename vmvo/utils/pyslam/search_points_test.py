...
def epiline_to_end_points(line, e, xinf, cols):
    ...
    step = min(
        Parameters.kMinDistanceFromEpipole, length
    ...
    if (
        not math.isnan(xmin[0])
        and not math.isnan(xmin[1])
        and xmin[0] > u0
        and xmin[0] < cols
    ):  # xmin is on the right of (u0,v0) => replace (u0,v0)
        u0 = xmin[0]
        v0 = xmin[1]

    if (
        not math.isnan(xmax[0])
        and not math.isnan(xmax[1])
        and xmax[0] < u1
        and xmax[0] > 0
    ):  # xmax is on the left of (u1,v1) => replace (u1,v1)
        u1 = xmax[0]
        v1 = xmax[1]
...
def find_matches_along_line(
    ...
    max_descriptor_distance = (
        0.5 * Parameters.kMaxDescriptorDistanceSearchEpipolar
    )
    ...
    for i in range(n + 1):
        ...
        for k_idx in f.kd.query_ball_point(x, radius):
            ...
            if f.get_point_match(k_idx) is None:
                descriptor_dist = Frame.descriptor_distance(
                    f.des[k_idx], descriptor
                )
                if descriptor_dist < max_descriptor_distance:
                    ...
                    if False:
                        if (
                            best_dist2 < max_descriptor_distance
                        ):  # stop once we have a second to best match
                            break
...
def search_frame_for_triangulation_test(f1, f2, img2, img1=None):
    ...
    for i, p in enumerate(f1.get_points()):
        if p is None:  # we consider just unmatched keypoints
            ...
            if (
                np.linalg.norm(kp - e1)
                < Parameters.kMinDistanceFromEpipole * scale_factor
            ):
                continue
            idxs1.append(i)
    kpsu1 = f1.kpsu[idxs1]
    ...
    for i, idx in enumerate(
        ...
        f2_idx, dist = find_matches_along_line(
            f2, e2, line_edges[i], f1.des[idx]
        )
        if f2_idx > -1:
            if not flag_match[f2_idx]:  # new match
                ...
                if (
                    dist < dist_match[f2_idx]
                ):  # update in case of a smaller distance
                    dist_match[f2_idx] = dist

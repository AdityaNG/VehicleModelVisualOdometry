...
def extract_multiscale(
    ...
    while s + 0.001 >= max(min_scale, min_size / max(H, W)):
        if s - 0.001 <= min(max_scale, max_size / max(H, W)):
            ...
            if verbose:
                print(
                    f"extracting at scale x{s:.02f} = {nw:4d}x{nh:3d} - level {level}"
                )
            # extract descriptors
            with torch.no_grad():
                res = net(imgs=[img])
            ...
            S.append(
                (32 / s) * torch.ones(n, dtype=torch.float32, device=d.device)
            )
            C.append(c)
            Q.append(q)
            D.append(d)
...
class R2d2Feature2D:
    def __init__(
        ...
        self.model_weights_path = (
            self.model_base_path + "/models/r2d2_WASF_N16.pt"
        )
    ...
    def detectAndCompute(self, frame, mask=None):  # mask is a fake input
        with self.lock:
            ...
            if kVerbose:
                print(
                    "detector: R2D2 , descriptor: R2D2 , #features: ",
                    len(self.kps),
                    ", frame res: ",
                    frame.shape[0:2],
                )
    ...
    def compute(
        ...
        with self.lock:
            if self.frame is not frame:
                Printer.orange(
                    "WARNING: R2D2 is recomputing both kps and des on last input frame",
                    frame.shape,
                )
                self.detectAndCompute(frame)
            return self.kps, self.des

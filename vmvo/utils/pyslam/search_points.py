...
def search_frame_by_projection(
    f_ref,
    f_cur,
    ...
    matched_ref_idxs = np.flatnonzero(
        (f_ref.points != None) & (f_ref.outliers == False)
    )
    ...
    if True:
        ...
        is_visible, projs, depths, dists = f_cur.are_visible(
            matched_ref_points
        )
    ...
    for i, p, j in zip(
        matched_ref_idxs, matched_ref_points, range(len(matched_ref_points))
    ):
        if not is_visible[j]:
            continue
        ...
        for kd_idx in kd_idxs[j]:
            ...
            if p_f_cur is not None:
                if (
                    p_f_cur.num_observations > 0
                ):  # we already matched p_f_cur => discard it
                    continue

            p_f_cur_octave = f_cur.octaves[kd_idx]
            if p_f_cur_octave < (kp_ref_octave - 1) or p_f_cur_octave > (
                kp_ref_octave + 1
            ):
                continue
...
def search_map_by_projection(
    ...
    for i, p in enumerate(points):
        if (
            not visible_pts[i] or p.is_bad
        ):  # point not visible in frame or is bad
            continue
        if (
            p.last_frame_id_seen == f_cur.id
        ):  # we already matched this map point to current frame or it was outlier
            continue
        ...
        for kd_idx in kd_idxs[i]:
            ...
            if (kp_level < predicted_level - 1) or (
        ...
        if best_dist < max_descriptor_distance:
            # apply match distance ratio test only if the best and second are in the same scale level
            if (best_level2 == best_level) and (
                best_dist > best_dist2 * ratio_test
            ):
                continue
...
def search_local_frames_by_projection(
    map, f_cur, local_window=Parameters.kLocalBAWindow
):
    # take the points in the last N frame
    points = []
    frames = map.keyframes[-local_window:]
    f_points = set(
        [p for f in frames for p in f.get_points() if (p is not None)]
    )
...
def search_frame_for_triangulation(
    ...
    if idxs1 is None or idxs2 is None:
        ...
        print(
            "search_frame_for_triangulation - matching - timer: ",
            timerMatch.elapsed(),
        )
    ...
    for i1, i2 in zip(idxs1, idxs2):
        if (
            kf1.get_point_match(i1) is not None
            or kf2.get_point_match(i2) is not None
        ...
        if (
            np.inner(delta, delta)
            < kMinDistanceFromEpipole2 * kp2_scale_factor
        ):  # OR.
...
def search_and_fuse(
    ...
    for i, p, j in zip(good_pts_idxs, good_pts, range(len(good_pts))):
        if (
            not good_pts_visible[j] or p.is_bad
        ):  # point not visible in frame or point is bad
            # print('p[%d] visible: %d, bad: %d' % (i, int(good_pts_visible[j]), int(p.is_bad)))
            continue

        if p.is_in_keyframe(
            keyframe
        ):  # we already matched this map point to this keyframe

...
class Parameters(object):
    ...
    # Keyframe generation
    kNumMinPointsForNewKf = (
        15  # minimum number of matched map points for spawning a new KeyFrame
    )
    kThNewKfRefRatio = 0.9  # for determining if a new KF must be spawned

    # Keyframe culling
    kKeyframeCullingRedundantObsRatio = 0.9
    kKeyframeMaxTimeDistanceInSecForCulling = (
        0.5  # [s]  # Use float('inf') for disabling it
    )
    ...
    kMatchRatioTestEpipolarLine = (
        0.8  # used just for test function find_matches_along_line()
    )
    #
    # Reference max descriptor distance (used for initial checks and then updated and adapted)
    kMaxDescriptorDistance = (
        0  # it is updated by feature_manager.py at runtime
    )
    ...
    #
    kMaxDescriptorDistanceSearchEpipolar = (
        0  # it is updated by feature_manager.py at runtime
    )

    # Local Mapping
    kLocalMappingNumNeighborKeyFrames = (
        20  #  [# frames]   for generating new points and fusing them
    )
    ...
    kEveryNumFramesLargeWindowBA = (
        10  # num of frames between two large window BA
    )

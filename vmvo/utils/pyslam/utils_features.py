...
from .utils_geom import add_ones, l2_distances, s1_diff_deg, s1_dist_deg
...
def descriptor_sigma_mad(des1, des2, descriptor_distances=l2_distances):
    dists = np.full(des1.shape[0], 0.0, dtype=np.float32)
...
def ssc_nms(
    kps, des, cols, rows, num_ret_points=Parameters.kNumFeatures, tolerance=0.1
):
    if len(kps) == 0:
        return kps, des
    ...
    while not complete:
        ...
        covered_vec = [
            [False for _ in range(num_cell_cols + 1)]
            for _ in range(num_cell_cols + 1)
        ]
        result = []

        for i in range(len(kps)):
            row = int(
                math.floor(kps[i].pt[1] / c)
        ...
        M = np.matrix(
            [
                ...
                [
                    +scale_sin,
                    +scale_cos,
                    (-scale_sin - scale_cos) * half_patch_size + y,
                ],
            ]
        )

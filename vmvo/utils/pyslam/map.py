...
class Map(object):
    ...
    def add_points(
        ...
        with self._lock:
            assert (
                kf1.is_keyframe and kf2.is_keyframe
            )  # kf1 and kf2 must be keyframes
            ...
            ratio_scale_consistency = (
                ...
                * Frame.feature_manager.scale_factor
            )
            ...
            patch_extension = (
                1 + 2 * delta
            )  # patch_extension x patch_extension
            ...
            for i, p in enumerate(points3d):
                ...
                try:
                    ...
                    color_patch = img1[
                        img_range[1][:, np.newaxis], img_range[0]
                    ]
                    # print('color_patch.shape:',color_patch.shape)

                    color = cv2.mean(color_patch)[
                        :3
                    ]  # compute the mean color in the patch
                except IndexError:
    ...
    def optimize(
        ...
        err = optimizer_g2o.bundle_adjustment(
            self.get_keyframes(),
            self.get_points(),
    ...
    def locally_optimize(
        self, kf_ref, verbose=False, rounds=10, abort_flag=g2o.Flag()
    ):
        keyframes, points, ref_keyframes = self.local_map.update(kf_ref)
        print(
            "local optimization window: ", sorted([kf.id for kf in keyframes])
        )
        print(
            "                     refs: ",
    ...
    def update_from_keyframes(self, local_keyframes):
        ...
        for p in good_points:
            ...
            for kf_viewing_p in p.keyframes():
                if (not kf_viewing_p.is_bad) and (
                    not kf_viewing_p in local_keyframes
                ):
...
class LocalWindowMap(LocalMapBase):
    ...
    def get_best_neighbors(
        self, kf_ref=None, N=Parameters.kLocalMappingNumNeighborKeyFrames
    ):
...
class LocalCovisibilityMap(LocalMapBase):
    ...
    def get_best_neighbors(
        self, kf_ref, N=Parameters.kLocalMappingNumNeighborKeyFrames
    ):

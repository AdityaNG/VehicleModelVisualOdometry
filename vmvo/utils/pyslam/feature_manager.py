...
# from .feature_orbslam2 import Orbslam2Feature2D
# from .feature_hardnet import HardnetFeature2D
# from .feature_geodesc import GeodescFeature2D
# from .feature_sosnet import SosnetFeature2D
# from .feature_l2net_keras import L2NetKerasFeature2D
# from .feature_l2net import L2NetFeature2D
...
# from .feature_contextdesc import ContextDescFeature2D
...
# from .feature_disk import DiskFeature2D
...
class FeatureManager(object):
    def __init__(
        ...
        self.layers_per_octave = (
            3  # for methods that uses octaves (SIFT, SURF, etc)
        )
        ...
        self.need_nms = (
            False  # need or not non-maximum suppression of keypoints
        )
        ...
        if self.descriptor_type in [
            ...
            Printer.orange(
                "forcing scale factor=2 for detector",
        ...
        if (
            ...
            raise ValueError(
                "Unknown feature detector %s" % self.detector_type
            )
        ...
        if self.is_detector_equal_to_descriptor:
            Printer.green(
                "using same detector and descriptor object: ",
            ...
            if (
                ...
                self.oriented_features = (
                    True  # SURF computes the keypoint orientation
                )
                ...
                self.oriented_features = (
                    True  # BRISK computes the keypoint orientation
                )
                ...
                self.oriented_features = (
                    True  # FREAK computes the keypoint orientation
                )
                ...
                raise ValueError(
                    "Unknown feature descriptor %s" % self.descriptor_type
                )
        ...
        try:
            ...
            Printer.red(
                "You did not set the norm type for: ",
                self.descriptor_type.name,
            )
            raise ValueError(
    ...
    def init_sigma_levels(self):
        ...
        for i in range(1, num_levels):
            self.scale_factors[i] = (
                self.scale_factors[i - 1] * self.scale_factor
            )

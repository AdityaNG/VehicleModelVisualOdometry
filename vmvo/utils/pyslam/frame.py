...
from .utils_geom import add_ones, normalize, poseRt
from .utils_sys import Printer, myjet
...
class FrameBase(object):
    ...
    def transform_points(self, points):
        with self._lock_pose:
            ...
            return (
                Rcw @ points.T + tcw
            ).T  # get points  w.r.t. camera frame  [Nx3]
    ...
    def is_visible(self, map_point):
        ...
        if (
            np.dot(PO, map_point.get_normal())
    ...
    def are_visible(self, map_points):
        ...
        out_flags = (
            are_in_image & are_in_good_view_angle & are_in_good_distance
        )
...
class Frame(FrameBase):
    ...
    def __init__(
        self, img, camera, pose=None, id=None, timestamp=None, kps_data=None
    ):
        ...
        self.kpsn = (
            None  # [n]ormalized keypoint coordinates     [Nx2] (Kinv * [kp,1])
        )
    ...
    def replace_point_match(self, p, idx):
        self.points[
            idx
        ] = p  # replacing is not critical (it does not create a 'None jump')
    ...
    def get_matched_inlier_points(self):
        with self._lock_features:
            matched_idxs = np.flatnonzero(
                (self.points != None) & (self.outliers == False)
            )
            matched_points = self.points[matched_idxs]
            return matched_points, matched_idxs

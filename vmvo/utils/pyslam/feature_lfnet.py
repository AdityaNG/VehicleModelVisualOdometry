...
from .utils_sys import Printer, is_opencv_version_greater_equal, print_options
...
def build_networks(lfnet_config, photo, is_training):
    ...
    heatmaps, det_endpoints = build_detector_helper(
        lfnet_config, detector, photo
    )
...
def build_detector_helper(lfnet_config, detector, photo):
    ...
    if lfnet_config.use_nms3d:
        heatmaps, det_endpoints = build_multi_scale_deep_detector_3DNMS(
            lfnet_config, detector, photo, reuse=False
        )
    else:
...
def build_lfnet_config():
    ...
    model_arg.add_argument(
        "--model",
        type=str,
        default=kLfNetModelPath,
        help="model file or directory",
    )
    model_arg.add_argument(
        "--top_k", type=int, default=500, help="number of keypoints"
    )
    model_arg.add_argument(
        "--max_longer_edge",
        type=int,
    ...
    if len(unparsed) > 0:
        raise ValueError(
            "Miss finding argument: unparsed={}\n".format(unparsed)
        )
    ...
    if os.path.isdir(tmp_config.model):
        ...
        config_path = os.path.join(
            os.path.dirname(tmp_config.model), "config.pkl"
        )
    try:
...
class LfNetFeature2D:
    def __init__(self, num_features=2000, do_tf_logging=False):
        ...
        self.photo_ph = tf.placeholder(
            tf.float32, [1, None, None, 1]
        )  # input grayscale image, normalized by 0~1
        is_training = tf.constant(False)  # Always False in testing

        self.ops = build_networks(
            self.lfnet_config, self.photo_ph, is_training
        )
        ...
        self.des = []
        self.frame = None
        self.keypoint_size = 20.0  # just a representative size for visualization and in order to convert extracted points to cv2.KeyPoint
    ...
    def compute_kps_des(self, photo):
        with self.lock:
            ...
            if (
                self.lfnet_config.max_longer_edge > 0
                and longer_edge > self.lfnet_config.max_longer_edge
            ):
                if height > width:
                    new_height = self.lfnet_config.max_longer_edge
                    new_width = int(
                        width * self.lfnet_config.max_longer_edge / height
                    )
                else:
                    new_height = int(
                        height * self.lfnet_config.max_longer_edge / width
                    )
            ...
            photo = (
                photo[None, ..., None].astype(np.float32) / 255.0
            )  # normalize 0-1
            ...
            if False:
                ...
                self.debug(
                    self.pts, scales, orientations, scale_maps, heatmaps
                )

            self.kps = convert_to_keypoints(
                self.pts,
                scales * self.keypoint_size,
    ...
    def detectAndCompute(self, frame, mask=None):  # mask is a fake input
        with self.lock:
            ...
            if kVerbose:
                print(
                    "detector: LFNET , descriptor: LFNET , #features: ",
                    len(self.kps),
                    ", frame res: ",
                    frame.shape[0:2],
                )
    ...
    def compute(
        self, frame, kps=None, mask=None
    ):  # kps is a fake input, mask is a fake input
        with self.lock:
            if self.frame is not frame:
                Printer.orange(
                    "WARNING: LFNET  is recomputing both kps and des on last input frame",
                    frame.shape,
                )
                self.detectAndCompute(frame)
            return self.kps, self.des

...
kLkPyrOpticFlowNumLevelsMin = (
    3  # maximal pyramid level number for LK optic flow
)
...
class LkFeatureTracker(FeatureTracker):
    def __init__(
        ...
        Printer.green(
            "LkFeatureTracker: num levels on LK pyr optic flow: ",
...
class DescriptorFeatureTracker(FeatureTracker):
    def __init__(
        ...
        if tracker_type == FeatureTrackerTypes.DES_FLANN:
            ...
            raise ValueError(
                "Unmanaged matching algo for feature tracker %s"
                % self.tracker_type
            )
    ...
    def track(self, image_ref, image_cur, kps_ref, des_ref):
        ...
        idxs_ref, idxs_cur = self.matcher.match(
            des_ref, des_cur

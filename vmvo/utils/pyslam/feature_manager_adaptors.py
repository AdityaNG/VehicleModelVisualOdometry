        ...
        if self.row_divs == 1 and self.col_divs == 1:
            ...
            if not self.do_parallel:
                ...
                with ThreadPoolExecutor(
                    max_workers=kBlockAdaptorMaxNumWorkers
                ) as executor:
                    executor.map(
...
class PyramidAdaptor(object):
    ...
    def initSigmaLevels(self):
        ...
        num_desired_features_per_level = (
            self.num_features
    ...
    def detect(self, frame, mask=None):
        if self.num_levels == 1:
            ...
            def detect_level(scale, pyr_cur, i):
                ...
                if kVerbose and False:
                    print(
                        "PyramidAdaptor - level", i, ", shape: ", pyr_cur.shape
                    )
                for kp in kps:
                    # print('kp.pt before: ', kp.pt)
                ...
                if self.do_sat_features_per_level:
                    kps, _ = sat_num_features(
                        kps, None, self.num_features_per_level[i]
                    )  # experimental
                kps_all.extend(kps)
    ...
    def detectAndCompute(self, frame, mask=None):
        if self.num_levels == 1:
            ...
            def detect_and_compute_level(
                scale, pyr_cur, pyr_cur_filtered, N, i
            ):
                kps = []
                if self.block_adaptor is None:

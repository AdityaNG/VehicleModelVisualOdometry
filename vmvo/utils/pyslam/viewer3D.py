...
class Viewer3D(object):
    ...
    def viewer_init(self, w, h):
        ...
        self.checkboxGrid = pangolin.VarBool(
            "ui.Grid", value=True, toggle=True
        )
        self.checkboxPause = pangolin.VarBool(
            "ui.Pause", value=False, toggle=True
        )
        ...
        self.int_slider = pangolin.VarInt(
            "ui.Point Size", value=kDefaultPointSize, min=1, max=10
        )
    ...
    def viewer_refresh(self, qmap, qvo, is_paused):
        ...
        if self.map_state is not None:
            ...
            if (
                self.map_state.predicted_pose is not None
                and kDrawCameraPrediction
            ):
            ...
            if len(self.map_state.points) > 0:
                ...
                # gl.glColor3f(1.0, 0.0, 0.0)
                pangolin.DrawPoints(
                    self.map_state.points, self.map_state.colors
                )

            if (
                self.map_state.reference_pose is not None
                and kDrawReferenceCamera
            ):

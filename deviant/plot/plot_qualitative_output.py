...
from deviant.lib.helpers.file_io import imread, imwrite
from deviant.lib.helpers.util import *

# from deviant.lib.math_3d import *
# from deviant.lib.util import create_colorbar, draw_bev, draw_tick_marks, imhstack, draw_3d_box
    ...
    if predictions_img is not None and predictions_img.size > 0:
        ...
        for j in range(N):
            ...
            if dataset == "nuscenes" or box_class in box_class_list:
                ...
                if show_3d:
                    verts_cur, corners_3d_cur = project_3d(
                        p2,
                        x3d[j],
                        y3d[j],
                        z3d[j],
...
parser.add_argument(
    ...
    type=str,
    default="kitti",
    help="one of kitti,nusc_kitti,nuscenes,waymo",
)
parser.add_argument(
    "--folder",
    type=str,
    default="output/gup_nuscenes/results_test/data",
    help="evaluate model on validation set",
)
parser.add_argument(
    "--folder2",
    type=str,
    default="output/prevolution_kitti_full/results_nusc_kitti_val/data",
)
...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="show ground truth 3D boxes in image for sanity check",
)
...
# ==================================================================================================
img_save_folder = (
    "images/qualitative/" + dataset
)  # "waymo_with_gt_segment-14739149465358076158_4740_000_4760_000_with_camera_labels_new"
if video_demo_flag:
...
print(
    "Choosing {} files out of {} files...".format(num_files_to_plot, num_files)
)
...
if video_demo_flag:
    ...
    file_index = np.sort(
        np.random.choice(range(num_files), num_files_to_plot, replace=False)
    )
...
for i in range(file_index.shape[0]):
    ...
    if dataset == "nuscenes":
        ...
        matched_index = [
            t
            for t in range(len(data_cache))
            if (
                os.path.basename(data_cache[t]["file_name"]).split(".")[0]
                == basename
            )
        ]
        ...
        p2_temp = np.array(data_cache[matched_index]["intrinsics"]).reshape(
            3, 3
        )
        p2 = np.eye(4)
        p2[:3, :3] = p2_temp
    ...
    canvas_bev = create_colorbar(
        bev_max_z * bev_scale, bev_w, color_lo=bev_c1, color_hi=bev_c2
    )
    ...
    if show_gt_in_image:
        plot_boxes_on_image_and_in_bev(
            gt_img,
            plot_color=color_gt,
            box_class_list=box_class_list,
            use_classwise_color=use_classwise_color,
            show_3d=True,
        )
    else:
    ...
    if show_baseline:
        ...
        predictions_img_2 = read_csv(
            predictions_file_2, ignore_warnings=True, use_pandas=True
        )
        plot_boxes_on_image_and_in_bev(
            predictions_img_2,
            plot_color=color_pred_2,
            box_class_list=box_class_list,
            show_3d=False,
            thickness=8,
        )
    ...
    if not show_gt_in_image:
        predictions_img = read_csv(
            pred_file, ignore_warnings=True, use_pandas=True
        )
        if predictions_img is not None and gt_img is not None:
            ...
            print(
                "Predictions = {} GT= {}".format(
                    predictions_img.shape[0], gt_img.shape[0]
                )
            )
    ...
    print(
        "Saving to {} with compression ratio {}".format(
            save_path, compression_ratio
        )
    )

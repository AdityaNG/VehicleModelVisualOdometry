...
from deviant.lib.helpers.file_io import read_numpy
from deviant.lib.helpers.util import get_downsampled_image
...
def get_equivariance_error(input, input2, scale=1):
    ...
    scaled_input = get_downsampled_image(
        img=input, curr_scale=scale, height_first_flag=False
    )
...
def get_stats_for_model(
    model, scale_1="scale_1", scale_ran="random_scaling", level=5
):
    rel_path = "results_kitti_val"
    scale_1_path = os.path.join(
        "output/" + model,
        rel_path + "/" + scale_1 + "/level_" + str(level) + ".npy",
    )
    scale_ran_path = os.path.join(
        "output/" + model,
        rel_path + "/" + scale_ran + "/level_" + str(level) + ".npy",
    )
    ...
    for i in range(num_images):
        ...
        curr_error = get_equivariance_error(
            scale_1_data[i], scale_ran_data[i], scale=curr_scale
        )
        if curr_error > 100:
            continue
    ...
    print(
        "Model= {:20s} Level= {} N= {} Mean= {:.2f} Std= {:.2f}".format(
            model,
            level,
            equiv_error_img.shape[0],
            np.mean(equiv_error_img),
            np.std(equiv_error_img),
        )
    )
...
plt.plot(
    x_arr, model1_mean, color=color_list[0], lw=fig_lw, label=label_list[0]
)
plt.plot(
    x_arr, model2_mean, color=color_list[1], lw=fig_lw, label=label_list[1]
)
plt.fill_between(
    x_arr,

...
ap.add_argument(
    ...
    "--input_folder",
    nargs="+",
    default=["output/run_221/result_kitti_val"],
...
ap.add_argument(
    ...
    nargs="+",
    default=["DEVIANT"],
    help="labels of the input folder",
)
args = ap.parse_args()
...
for i, dis in enumerate(dis_keys):
    roc_1.append(
        [
            m3d_res["res_{}m_{}".format(dis, iou)]["det_3d_" + lbl][1]
            for iou in iou_keys
        ]
    )
for j in range(num_input_folders):
    ...
    for i, dis in enumerate(dis_keys):
        roc_2[j].append(
            [
                our_res[j]["res_{}m_{}".format(dis, iou)]["det_3d_" + lbl][1]
                for iou in iou_keys
            ]
        )
...
patch4 = Line2D(
    [0], [0], color="black", lw=fig_lw, label=label_folder1, linestyle="--"
)
...
save_path = os.path.join(
    "images",
    "AP3D_vs_IOU3D_threshold_at_different_gt_distances_" + lbl + ".png",
)
...
save_path = os.path.join(
    "images",
    "AP3D_log_vs_IOU3D_threshold_at_different_gt_distances_" + lbl + ".png",
)
savefig(plt, save_path, tight_flag=True, pad_inches=pad_inches)
plt.close()

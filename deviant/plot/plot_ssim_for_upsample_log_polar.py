...
from deviant.lib.projective.log_polar_conv import (
    cartesian_to_log_polar_feature_map,
    convert_CHW_to_HWC,
    convert_HWC_to_CHW,
    log_polar_to_cartesian_feature_map,
    our_interpolate,
)


def get_error_stats(
    input1_hwc, input2_hwc, prefix="", ignore_black_area=False
):
...
def numpy_log_polar(input_np_height_curr, center_orig, h_orig, upsample=1.0):
    ...
    if upsample != 1.0:
        ...
        input_np_height_curr = cv2.resize(
            input_np_height_curr,
            (int(w_curr * upsample), int(h_curr * upsample)),
        )
        center_curr = center_curr * upsample

    input_np_channel_curr = convert_HWC_to_CHW(input_np_height_curr)
    log_polar_np_channel = cartesian_to_log_polar_feature_map(
        input_np_channel_curr, center_cartesian_feature_map=center_curr
    )
    input_back_np_channel = log_polar_to_cartesian_feature_map(
        log_polar_np_channel, center_cartesian_feature_map=center_curr
    )
    input_back_np_height = convert_CHW_to_HWC(input_back_np_channel)

    if upsample != 1.0:
        # Downsample, cv2.resize operates in Height space
        input_back_np_height = cv2.resize(
            input_back_np_height, (w_curr, h_curr)
        )

    return log_polar_np_channel, input_back_np_height


def pytorch_log_polar(
    input_ten_channel_curr,
    center_ten_orig,
    h_orig,
    upsample_factor_for_interpolate=1.0,
    mode="bilinear",
    ...
    if upsample_factor_for_interpolate != 1.0:
        input_ten_channel_curr = our_interpolate(
            input_ten_channel_curr,
            scale_factor=upsample_factor_for_interpolate,
            mode=mode,
        )
        center_ten_curr = center_ten_curr * upsample_factor_for_interpolate

    log_polar_ten_channel = cartesian_to_log_polar_feature_map(
        input_ten_channel_curr,
        center_cartesian_feature_map=center_ten_curr,
        interpolation_method=interpolation_method,
        upsample_factor_for_log_polar_output=upsample_factor_for_log_polar_output,
    )
    input_back_ten_channel = log_polar_to_cartesian_feature_map(
        log_polar_ten_channel,
        center_cartesian_feature_map=center_ten_curr,
        interpolation_method=interpolation_method,
        upsample_factor_for_log_polar_output=upsample_factor_for_log_polar_output,
    )
    ...
    if upsample_factor_for_interpolate != 1.0:
        input_back_ten_channel = our_interpolate(
            input_back_ten_channel,
            scale_factor=1.0 / upsample_factor_for_interpolate,
            mode=mode,
        )
...
"""
ssim_stats = read_numpy(
    path="images/ssim_stats_for_upsample_log_polar_bkp.npy"
)
...
color_list = [
    params.color_set1_cyan / 255.0,
    params.color_set1_pink / 255.0,
    params.color_set1_yellow / 255.0,
]
...
for i in [2, 1]:
    for j in range(num_upsample):
        ...
        if j == 2:
            label_text = modes_list[
                i
            ].title()  # + " x" + str(int(upsample_list[j]))
            plt.plot(
                input_res_np,
                mean_plot,
...
patch2 = mpatches.Patch(
    color=color_list[1], label="PyTorch " + modes_list[1].title()
)
patch3 = mpatches.Patch(
    color=color_list[2], label="PyTorch " + modes_list[2].title()
)
patch4 = Line2D(
    [0],
    [0],
    color="black",
    lw=fig_lw,
    label="Up x" + str(int(upsample_list[0])),
...
patch5 = Line2D(
    ...
    color="black",
    lw=fig_lw,
    label="Up x" + str(int(upsample_list[1])),
...
patch6 = Line2D(
    ...
    color="black",
    lw=fig_lw,
    label="Up x" + str(int(upsample_list[2])),
...
plt.legend(
    ...
    loc="lower right",
    fontsize=legend_fs,
    borderaxespad=legend_border_axes_plot_border_pad,
    borderpad=legend_border_pad,
    labelspacing=legend_vertical_label_spacing,

...
scale_sets = [
    [0.83, 0.9, 1.0]
]  # [[0.9, 1.26, 1.76]]#[[0.83, 1.0, 1.2], [0.9, 1.26, 1.76], [0.5, 1.0, 1.5]]
# ===================================================================
# Effective size 3
# ===================================================================
...
print(
    "Showing for {} scale sets each one with scales x filters= {} x {}".format(
        num_scale_sets, num_scales, num_filters
    )
)
fig = plt.figure(
    figsize=(params.size[0] // 2, params.size[1] // 2), dpi=params.DPI * 3
)
...
for k in range(num_scale_sets):
    for i in range(num_scales):
        for j in range(num_filters):
            scale_curr = i
            plt.subplot(
                num_scales * num_scale_sets,
                num_filters,
                k * num_filters * num_scales + i * num_filters + j + 1,
            )

            if k == 0:
                plt.imshow(
                    basis1_np[j][scale_curr], vmin=vmin, vmax=vmax, cmap=cmap
                )
...
cmap = diverge_map(
    params.color_set1_pink / 255.0, params.color_set1_cyan / 255.0
)

...
def get_img_features(model, scale_name_list, level):
    ...
    for i, scale_name in enumerate(scale_name_list):
        scale_npy_path = os.path.join(
            "output/" + model,
            rel_path + "/val_scale/level_" + str(level) + ".npy",
        )
...
def get_error(ref_image, prev_image, masking=False):
    ...
    scaled_input_base_cnn = get_downsampled_image(
        img=input_base_cnn, curr_scale=curr_scale, height_first_flag=False
    )[0]
    input2_cnn = prev_image
    if masking:
        left_x, right_x, top_y, bottom_y = get_bounds(
            img_shape_x=input_base_cnn.shape[2],
            img_shape_y=input_base_cnn.shape[1],
            curr_scale=curr_scale,
        )
        scaled_input_base_cnn = scaled_input_base_cnn[
            top_y:bottom_y, left_x:right_x
        ]
        input2_cnn = input2_cnn[top_y:bottom_y, left_x:right_x]

    error = norm(scaled_input_base_cnn - input2_cnn) / (
        norm(scaled_input_base_cnn) + 0.01
    )
    error_full_img = np.abs(scaled_input_base_cnn - input2_cnn) / (
        np.abs(scaled_input_base_cnn) + 0.01
    )
...
output_cnn = get_img_features(
    model=model_cnn, scale_name_list=key, level=level
)
output_deviant = get_img_features(
    model=model_deviant, scale_name_list=key, level=level
)
...
for i in range(frame_diff, num_frames):
    ...
    plt.subplot(313)
    plt.imshow(
        error_full_deviant, vmin=vmin, vmax=vmax, cmap=cmap, alpha=alpha
    )
    plt.xticks([])
    plt.yticks([])
    ...
    cbar_ticks = np.around(
        np.array([0, vmax / 4, vmax / 2, 3 * vmax / 4, vmax - 0.1]), decimals=1
    )

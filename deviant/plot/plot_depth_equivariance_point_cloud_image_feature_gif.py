...
from plot.common_operations import (
    add_ubyte_image_to_gif_writer,
    close_gif_writer,
    convert_fig_to_ubyte_image,
    open_gif_writer,
)

from deviant.lib.helpers.file_io import imread, write_lines
...
image_file = os.path.join(
    folder, "images/depth_equivariance_with_projective.tex"
)
num_pts = 101
...
for cnt in range(1, num_pts):
    ...
    lines += (
        "\\node[inner sep=0pt, thick] (input) at (-12.3,-6) {\includegraphics[trim={1.0cm 1.2cm 4.3cm 0.2cm}, clip, height="
        + str(pc_linspace[cnt])
        + "cm]{images/000000.png}};\n\n"
    )
    ...
    lines += (
        "\\node[inner sep=0pt] (input) at (0,-6.0) {\includegraphics[trim={5cm 0 5cm 0}, clip, height="
        + str(img_linspace[cnt])
        + "cm]{images/000008.png}};\n\n"
    )
    ...
    lines += (
        "\\node[inner sep=0pt, thick] (input) at (15,-6) {\includegraphics[trim={2.2cm 0 2.2cm 0}, clip, height="
        + str(img_linspace[cnt])
        + "cm]{images/filter_1.png}};\n\n"
    )
    ...
    # convert pdf to png
    command = (
        "convert -density 600 -trim -alpha remove depth_equivariance.pdf "
        + output_folder
        + "/"
        + str(cnt).zfill(6)
        + ".png"

...
def get_filter_on_image(h, w, filter_type="vanilla"):
    ...
    color1 = (
        params.color_blue
    )  # params.color_set1_cyan#np.array([30, 255, 255])
    color2 = params.color_set1_cyan  # np.array([30, 144, 255])
    color5 = (
        params.color_red
    )  # params.color_set1_pink#np.array([255, 30, 143])
    ...
    for i in range(h // skip_h):
        ...
        for j in range(w // skip_w):
            ...
            if (cnt + i) % 2 == 0:
                filter_on_image[
                    curr_i:curr_i_fin, curr_j:curr_j_fin
                ] = color_temp_1
            else:
                filter_on_image[
                    curr_i:curr_i_fin, curr_j:curr_j_fin
...
conv_on_image_conventional = get_filter_on_image(
    h=h, w=w, filter_type="vanilla"
)
conv_on_image_ours = cv2.logPolar(
    conv_on_image_conventional,
    center=(cX, cY),
    M=min(cX, cY),

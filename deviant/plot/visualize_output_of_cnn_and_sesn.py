...
print(
    torch.norm(
        model[0].basis[basis_index, 1] - model2[0].basis[basis_index, 1]
    )
)
# print(torch.norm(model[0].basis[basis_index, 2] - model2[0].basis[basis_index, 1]))
print(
    torch.norm(
        model2[0].basis[basis_index, 0] - model2[0].basis[basis_index, 1]
    )
)
print(
    torch.norm(
        model2[0].basis[basis_index, 1] - model2[0].basis[basis_index, 2]
    )
)
...
img_float = torch.Tensor(
    img_numpy.transpose(2, 0, 1)[0][np.newaxis,] / 255.0
).unsqueeze(0)
...
if plot_gif:
    gif_file_path = (
        "images/ses_in_cont_domain_kernel_size_"
        + str(kernel_size)
        + "_basis_"
        + str(basis_index)
        + ".gif"
...
for i, scale_curr in enumerate(scale_list):
    img_np_new = get_downsampled_image(
        img=img_numpy, curr_scale=1.0 / scale_curr
    )
    img_float_new = torch.Tensor(
        img_np_new.transpose(2, 0, 1)[0][np.newaxis,] / 255.0
    ).unsqueeze(0)
    ...
    if show_downsampled:
        ...
        plt.imshow(
            get_downsampled_image(
                feature_img[0][np.newaxis].repeat(3, axis=0),
                curr_scale=1.0 / scale_curr,
                height_first_flag=False,
            )[0],
            cmap=cmap,
            vmin=cmap_min,
            vmax=cmap_max,
        ...
        plt.title(
            r"$T_{"
            + "{:.1f}".format(scale_curr)
            + "}(h*\Psi_{"
            + "{:.1f}".format(sesn_scales[0])
            + r"})$"
        )
        plt.axis("off")
        ...
        plt.imshow(
            get_downsampled_image(
                feature_img[1][np.newaxis].repeat(3, axis=0),
                curr_scale=1.0 / scale_curr,
                height_first_flag=False,
            )[0],
            cmap=cmap,
        ...
        if feature_img.shape[0] > 2:
            ...
            plt.imshow(
                get_downsampled_image(
                    feature_img[2][np.newaxis].repeat(3, axis=0),
                    curr_scale=1.0 / scale_curr,
                    height_first_flag=False,
                )[0],
                cmap=cmap,

...
def foo(x, y):
    ...
    lines += (
        "\\node[inner sep=0pt] (input) at ("
        + str(x)
        + ","
        + str(-6.0 + y)
        + ") {\includegraphics[trim={5cm 0 5cm 0}, clip, height="
        + "1.3"
    ...
    lines += (
        "\\node[inner sep=0pt, thick] (input) at ("
        + str(15 + x)
        + ","
        + str(-6.0 + y)
        + ") {\includegraphics[trim={2.3cm 0 2.3cm 0}, clip, height="
        + "1.3"
    ...
    # convert pdf to png
    command = (
        "convert -density 600 -trim -alpha remove cnn_equivariance.pdf "
        + output_folder
        + "/"
        + str(cnt).zfill(6)
        + ".png"

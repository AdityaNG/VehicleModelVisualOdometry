...
points = np.fromfile(point_cloud_path, dtype=np.float32).reshape(
    -1, 4
)  # Assumes XYZI format (4 channels)
...
valid_index = np.logical_and(
    np.logical_and(
        np.logical_and(
            np.logical_and(
                pixel_locations[:, 2] >= -0.5, pixel_locations[:, 0] >= -th
            ),
            pixel_locations[:, 0] <= image_width - 1 + th,
        ),
        pixel_locations[:, 1] >= -th,
    ),
    pixel_locations[:, 1] <= image_height - 1 + th,
)
...
colored_point_cloud.points = o3d.utility.Vector3dVector(
    points[:, :3]
)  # XYZ coordinates
colored_point_cloud.colors = o3d.utility.Vector3dVector(
    colors / 255.0

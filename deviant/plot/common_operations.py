...
# from deviant.lib.util import get_correlation, combine
# from deviant.lib.math_3d import project_3d_points_in_4D_format, project_3d_corners
# from deviant.lib.core import iou
# from deviant.lib.core import iou3d
from deviant.lib.helpers.file_io import pickle_read, pickle_write, read_csv

sub_index = 1


def savefig(
    plt, path, show_message=True, tight_flag=True, pad_inches=0, newline=True
):
    if show_message:
...
def diverge_map(
    top_color_frac=(1.0, 0.0, 0.25), bottom_color_frac=(1.0, 0.906, 0.04)
):
    """
    ...
    if bottom_color_frac is not None:
        ...
        newcolors2 = np.vstack(
            (
                bottom_color_cmp(np.linspace(0, 1, 128)),
                top_color_cmp(np.linspace(1, 0, 128)),
            )
        )
    else:
...
def throw_samples(
    x, y, frac_to_keep=0.9, show_message=True, throw_around_center=True
):
    """
    ...
    if throw_around_center:
        ...
        keep_index = sorted_indices[
            (center_index - samples_to_keep // 2) : (
                center_index + samples_to_keep // 2
            )
            + 1
        ]
...
def plot_one_error_with_box_error(
    y,
    error_in_box,
    name="y",
    plt=None,
    show_message=False,
    throw_samples_flag=True,
    ...
    if throw_samples_flag:
        # Throw the outliers
        error_in_box, y = throw_samples(
            error_in_box, y, show_message=show_message
        )
    ...
    plt.plot(
        ...
        color=color,
        lw=params.lw,
        label=label + ", Corr={:.3f}".format(correlation),
    )
...
def plot_one_set_of_error_plots_with_box_error(
    error_variable_x_axis,
    z3d_gt,
    score,
    error_in_x1,
    error_in_y1,
    ...
    if (
        error_in_x3d_2d is None
        or error_in_y3d_2d is None
        or error_in_z3d_2d is None
    ):
        plot_more = False
    else:
        plot_more = True
    ...
    plot_one_error_with_box_error(
        score,
        error_variable_x_axis,
        name=r"class conf ",
        plt=plt,
        show_message=True,
        throw_samples_flag=throw_samples_flag,
        iou_on_x=iou_on_x,
    )
    ...
    if save_path is None:
        save_path = get_output_file_path(
            prefix="pred_error_vs",
            postfix="",
            relative=relative,
            threshold=threshold_score,
            before_nms_flag=before_nms_flag,
            folder=params.IMAGE_DIR,
        )
    savefig(plt, save_path)
    plt.close()


def read_folder_and_get_all_errors(
    input_folder,
    prediction_folder_relative,
    num_images=-1,
    ...
    prediction_files = sorted(
        glob.glob(
            os.path.join(input_folder, prediction_folder_relative + "/*.txt")
        )
    )
    ...
    print(
        "Choosing {} files out of {} prediction files for plotting".format(
            num_images, num_prediction_files
        )
    )
    ...
    file_index = np.sort(
        np.random.choice(
            range(num_prediction_files), num_images, replace=False
        )
    )
    ...
    for i in range(num_images):
        ...
        if predictions_img.size > 0 and gt_img is not None:
            ...
            predictions_centers_3d_2d = project_3d_points_in_4D_format(
                p2, predictions_centers_3d, pad_ones=True
            )
            predictions_centers_3d_2d = predictions_centers_3d_2d[:3].T
            predictions_img = np.hstack(
                (predictions_img, predictions_centers_3d_2d)
            )

            # Add projected 3d center information to gt_img
            gt_centers_3d = gt_img[:, 10:13].T
            gt_centers_3d_2d = project_3d_points_in_4D_format(
                p2, gt_centers_3d, pad_ones=True
            )
            ...
            if gt_img.shape[0] > 0:
                ...
                suff_overlap_ind = np.where(max_overlaps > min_iou2d_overlap)[
                    0
                ]
                ...
                for j in range(N):
                    _, corners_3d_b1 = project_3d_corners(
                        p2,
                        predictions_img[j, 10],
                        predictions_img[j, 11],
                        predictions_img[j, 12],
                        w3d=predictions_img[j, 8],
                        h3d=predictions_img[j, 7],
                        l3d=predictions_img[j, 9],
                        ry3d=predictions_img[j, 13],
                        iou_3d_convention=True,
                    )  # ,
                    # return_in_4D=False)
                    corners_3d_b1 = corners_3d_b1[:3]
                    _, corners_3d_b2 = project_3d_corners(
                        p2,
                        gt_matched_img[j, 10],
                        gt_matched_img[j, 11],
                        gt_matched_img[j, 12],
                        w3d=gt_matched_img[j, 8],
                        h3d=gt_matched_img[j, 7],
                        l3d=gt_matched_img[j, 9],
                        ry3d=gt_matched_img[j, 13],
                        iou_3d_convention=True,
                    )  # ,
                    # return_in_4D=False)
...
def open_gif_writer(file_path, duration=0.5, loop=0):
    ...
    if loop is None:
        ...
        gif_writer = imageio.get_writer(
            file_path, mode="I", duration=duration, loop=loop
        )

    ...
    def __init__(self, root_dir, split, cfg):
        ...
        if self.eval_dataset == "kitti":
            ...
            self.data_dir = os.path.join(
                root_dir, "KITTI", "testing" if split == "test" else "training"
            )
            ...
            split_dir = os.path.join(
                root_dir, self.eval_dataset, "ImageSets", split + ".txt"
            )
            self.data_dir = os.path.join(
                root_dir,
                self.eval_dataset,
    ...
    def __getitem__(self, item):
        ...
        if self.split != "test":
            ...
            for i in range(object_num):
                ...
                if (
                    objects[i].cls_type == "Car"
                    and num_lidar_points_ber_box <= 100
                ):
                    continue
                if (
                    objects[i].cls_type != "Car"
                    and num_lidar_points_ber_box <= 50
                ):
                    continue

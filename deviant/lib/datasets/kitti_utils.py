...
class Object3d(object):
    ...
    def to_bev_box2d(self, oblique=True, voxel_size=0.1):
        ...
        if oblique:
            ...
            box2d[:, 0] = (
                (xz_corners[:, 0] - Object3d.MIN_XZ[0]) / voxel_size
            ).astype(np.int32)
            box2d[:, 1] = (
                Object3d.BEV_SHAPE[0]
                - 1
                - (
                    (xz_corners[:, 1] - Object3d.MIN_XZ[1]) / voxel_size
                ).astype(np.int32)
            )
            ...
            cu = np.floor(
                (self.pos[0] - Object3d.MIN_XZ[0]) / voxel_size
            ).astype(np.int32)
            cv = (
                Object3d.BEV_SHAPE[0]
                - 1
            ...
            half_l, half_w = int(self.l / voxel_size / 2), int(
                self.w / voxel_size / 2
            )
...
class Calibration(object):
            ...
            np.concatenate(
                [
                    np.expand_dims(
                        np.tile(
                            np.expand_dims(
                                np.linspace(0, img_size[0], wsize), 0
                            ),
                            [hsize, 1],
                        ),
                        -1,
                    ...
                    np.expand_dims(
                        np.tile(
                            np.expand_dims(
                                np.linspace(0, img_size[1], hsize), 1
                            ),
                            [1, wsize],
                        ),
                        -1,
    ...
    def affine_transform(self, img_size, trans):
        ...
        p3ds = self.img_to_rect(p2ds[:, 0:1], p2ds[:, 1:2], p2ds[:, 2:3])
        p2ds[:, :2] = np.dot(
            np.concatenate([p2ds[:, :2], np.ones([wsize * hsize, 1])], -1),
            trans.T,
        )
...
if __name__ == "__main__":
    ...
    for object in objects:
        ...
        print(
            "alpha ---> ry ",
            object.alpha,
            calib.alpha2ry(object.alpha, center_2d[0]),
        )
        break

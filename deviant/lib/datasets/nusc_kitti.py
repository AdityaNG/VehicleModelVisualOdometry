...
from deviant.lib.datasets.utils import (
    angle2class,
    check_range,
    draw_umich_gaussian,
    gaussian_radius,
    get_angle_from_box3d,
)
...
class NUSC_KITTI(data.Dataset):
    def __init__(self, root_dir, split, cfg):
        ...
        self.cls_min_lidar_pts = {
            "barrier": 10,
            "bicycle": 5,
            "bus": 40,
            "car": 35,
            "construction_vehicle": 20,
            "motorcycle": 10,
            "pedestrian": 5,
            "traffic_cone": 4,
            "trailer": 40,
        ...
        self.box2d_orig_hmin = (
            20
            if "box2d_orig_hmin" not in cfg.keys()
            else cfg["box2d_orig_hmin"]
        )

        ##w,h,l
        self.cls_mean_size = np.array(
            [
                [0.9856928278, 2.5350272733, 0.4964593291],
        ...
        split_dir = os.path.join(
            root_dir, "nusc_kitti", "ImageSets", split + ".txt"
        )
        ...
        self.data_dir = os.path.join(
            root_dir,
            "nusc_kitti",
            "validation" if "val" in split else "training",
        )
        ...
        self.data_augmentation = (
            True if split in ["train", "train_small", "trainval"] else False
        )
    ...
    def __getitem__(self, item):
        ...
        if self.split != "test":
            ...
            object_num = (
                len(objects) if len(objects) < self.max_objs else self.max_objs
            )
            for i in range(object_num):
                ...
                if (
                    objects[i].pos[-1]
                    > self.cls_max_distance[objects[i].cls_type]
                ):
                    continue
                ...
                if (
                    center_heatmap[0] < 0
                    or center_heatmap[0] >= features_size[0]
                ):
                    continue
                if (
                    center_heatmap[1] < 0
                    or center_heatmap[1] >= features_size[1]
                ):
                    continue
                ...
                indices[i] = (
                    center_heatmap[1] * features_size[0] + center_heatmap[0]
                )
                ...
                if (
                    num_lidar_points_per_box
                    <= self.cls_min_lidar_pts[objects[i].cls_type]
                ):
                    continue

...
def focal_loss(input, target, alpha=0.25, gamma=2.0):
    ...
    pos_loss = (
        torch.log(input) * torch.pow(1 - input, gamma) * pos_inds * alpha
    )
    neg_loss = (
        torch.log(1 - input) * torch.pow(input, gamma) * neg_inds * (1 - alpha)
    )
...
def focal_loss_cornernet(input, target, gamma=2.0):
    ...
    neg_loss = (
        torch.log(1 - input) * torch.pow(input, gamma) * neg_inds * neg_weights
    )

...
from deviant.lib.helpers.decode_helper import _transpose_and_gather_feat
from deviant.lib.losses.focal_loss import focal_loss_cornernet as focal_loss
from deviant.lib.losses.uncertainty_loss import (
    laplacian_aleatoric_uncertainty_loss,
)
...
class Hierarchical_Task_Learning:
    def __init__(self, epoch0_loss, stat_epoch_nums=5):
        ...
        self.term2index = {
            term: self.index2term.index(term) for term in self.index2term
        }  # term2index
    ...
    def compute_weight(self, current_loss, epoch):
        ...
        eval_loss_input = torch.cat(
            [_.unsqueeze(0) for _ in current_loss.values()]
        ).unsqueeze(0)
        for term in self.loss_graph:
            if len(self.loss_graph[term]) == 0:
                loss_weights[term] = torch.tensor(1.0).to(
                    current_loss[term].device
                )
            else:
                loss_weights[term] = torch.tensor(0.0).to(
                    current_loss[term].device
                )
        # update losses list
    ...
    def update_e0(self, eval_loss):
        self.epoch0_loss = torch.cat(
            [_.unsqueeze(0) for _ in eval_loss.values()]
        ).unsqueeze(0)
...
class GupnetLoss(nn.Module):
    ...
    def compute_segmentation_loss(self, input, target):
        input["heatmap"] = torch.clamp(
            input["heatmap"].sigmoid_(), min=1e-4, max=1 - 1e-4
        )
    ...
    def compute_bbox2d_loss(self, input, target):
        if target["mask_2d"].sum() <= 0:
            ...
            size2d_target = extract_target_from_tensor(
                target["size_2d"], target["mask_2d"]
            )
            size2d_loss = F.l1_loss(
                size2d_input, size2d_target, reduction="mean"
            )
            # compute offset2d loss
            offset2d_input = extract_input_from_tensor(
                input["offset_2d"], target["indices"], target["mask_2d"]
            )
            offset2d_target = extract_target_from_tensor(
                target["offset_2d"], target["mask_2d"]
            )
    ...
    def compute_bbox3d_loss(self, input, target, mask_type="mask_2d"):
        if target[mask_type].sum() <= 0:
            ...
            depth_input, depth_log_variance = (
                depth_input[:, 0:1],
                depth_input[:, 1:2],
            )
            depth_target = extract_target_from_tensor(
                target["depth"], target[mask_type]
            )
            ...
            offset3d_target = extract_target_from_tensor(
                target["offset_3d"], target[mask_type]
            )
            ...
            size3d_target = extract_target_from_tensor(
                target["size_3d"], target[mask_type]
            )
            size3d_loss = (
                F.l1_loss(
                    size3d_input[:, 1:], size3d_target[:, 1:], reduction="mean"
                )
                * 2
                / 3
                + laplacian_aleatoric_uncertainty_loss(
                    size3d_input[:, 0:1],
                    size3d_target[:, 0:1],
                    input["h3d_log_variance"][input["train_tag"]],
                )
                / 3
            )
...
def compute_heading_loss(input, mask, target_cls, target_reg):
    ...
    cls_onehot = (
        torch.zeros(target_cls.shape[0], 12)
        .cuda()
        .scatter_(dim=1, index=target_cls.view(-1, 1), value=1)
    )

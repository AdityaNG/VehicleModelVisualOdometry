...
def copy_state_dict_bn(dict_target, dict_origin, key_target, key_origin):
    for postfix in ["weight", "bias", "running_mean", "running_var"]:
        dict_target[key_target + "." + postfix] = dict_origin[
            key_origin + "." + postfix
        ]


def copy_state_dict_conv_hh_1x1(
    dict_target, dict_origin, key_target, key_origin
):
...
def copy_state_dict_conv_hh_1x1_interscale(
    dict_target, dict_origin, key_target, key_origin
):
...
def copy_state_dict_conv_zh(
    dict_target, dict_origin, key_target, key_origin, scale=0, eps=1e-12
):
    weight = dict_origin[key_origin + ".weight"]
    basis = dict_target[key_target + ".basis"][:, scale]
    logging.info(
        "Src weight= {}, basis= {}, target weight= {} ".format(
            get_tensor_shape_as_string(weight),
            get_tensor_shape_as_string(basis),
            get_tensor_shape_as_string(dict_target[key_target + ".weight"]),
        )
    )

    dict_target[key_target + ".weight"] = _approximate_weight(
        basis, weight, eps
    )
...
def _approximate_weight(basis, target_weight, eps=1e-12):
    ...
    with torch.no_grad():
        basis = F.pad(
            basis, [(w - W) // 2, (w - W) // 2, (h - W) // 2, (h - H) // 2]
        )
        target_weight = (
            target_weight.view(C_out * C_in, h * w).detach().cpu().numpy()
        )
    ...
    if matrix_rank == basis.shape[0]:
        ...
        print(
            "  !!! basis is incomplete. rank={} < num_funcs={}. "
...
def get_layers_name_type_replace_flag(
    dest_input, replace_all=True, replace_layer_names=None, replace_style=None
):
    """
    ...
    dest_layers_repr = [
        convert_param_name_to_layer(name) for name in dest_keys
    ]
    ...
    for layer_name, layer_type in dest_layers_repr:
        if max_scale_before_after_transition:
            ...
            if (
                "transition" in layer_name
                or "norm5" in layer_name
                or "conv1" in layer_name
                or "norm1" in layer_name
            ):
                layer_replace_flag = False
            elif replace_all:
                layer_replace_flag = True
        ...
        dest_layers_name_type_replace_flag.append(
            (layer_name, layer_type, layer_replace_flag)
        )
...
def calculate_new_weights(
    ...
    dest_keys = list(dest_state_dict.keys())
    dest_layers = list(
        set([".".join(key.split(".")[:-1]) for key in dest_keys])
    )
    dest_layers_repr = [
        convert_param_name_to_layer(name) for name in dest_keys
    ]
    ...
    dest_layers_name_type_replace_flag = get_layers_name_type_replace_flag(
        dest_input,
    ...
    for i, (layer_name, layer_type, layer_replace_flag) in enumerate(
        dest_layers_name_type_replace_flag
    ):
        ...
        if not layer_replace_flag:
            pass
            logging.info(
                "{:40s} {:10s} copy src_input weights      ".format(
            ...
            logging.info(
                "{:40s} {:10s} transform src_input weights ".format(
                    layer_name, layer_type
                )
            )
            ...
            if src_key not in src_layer_names:
                for src_key_curr in src_layer_names:
                    ...
                    if (
                        src_key_curr == dest_key.replace(".extra0", "")
            ...
            if layer_type == "conv":
                ...
                if layer_replace_flag:
                    ...
                    if weight.shape[-1] == weight.shape[-2] == 1:
                        ...
                        copy_state_dict_conv_hh(
                            dest_state_dict,
                            src_state_dict,
                            key_target=dest_key,
                            key_origin=src_key,
                            scale=scale_index_for_init,
                        )
                    else:
                        copy_state_dict_conv_zh(
                            dest_state_dict,
                            src_state_dict,
                            key_target=dest_key,
                            key_origin=src_key,
                    ...
                    copy_state_dict_conv_2d(
                        dest_state_dict, src_state_dict, dest_key, src_key
                    )

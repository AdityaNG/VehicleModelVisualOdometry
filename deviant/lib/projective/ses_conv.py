...
from .ses_basis import normalize_basis_by_min_scale, steerable_A
...
def get_sqroot_patch_energy(input, kernel_size, padding, stride=1, norm_p=2.0):
    ...
    pool_filters = cast_to_cpu_cuda_tensor(
...
def convolution_with_normalized_patches(
    input,
    kernel_wt,
    padding=0,
    stride=1,
    ...
    if normalize_filters:
        norm_kernel_wt = F.normalize(
            kernel_wt.reshape((-1, K * K)), dim=1
        ).reshape(
            (out_channels, in_channels_grp, K, K)
        )  # Cout x Cin x H x W
    else:
    ...
    if normalize_patches:
        sqroot_patch_energy = get_sqroot_patch_energy(
            input, kernel_size=K, padding=padding, stride=stride, norm_p=norm_p
        )  # B x Cin x Hout x Wout

    output = cast_to_cpu_cuda_tensor(
        torch.zeros((batch, out_channels, Hout, Wout)), reference_tensor=input
    )
    ...
    for i in range(groups):
        ...
        norm_kernel_wt_interm = norm_kernel_wt[
            index_output[i] : index_output[i + 1]
        ].reshape(out_channels_grp * in_channels_grp, 1, K, K)
        output_grp = F.conv2d(
            inp_group,
            norm_kernel_wt_interm,
            padding=padding,
            stride=stride,
            groups=in_channels_grp,
        )  # B x Cout*Cin x Hout x Wout
        output_grp = (
            output_grp.view(
                batch, in_channels_grp, out_channels_grp, Hout, Wout
            )
            .permute(0, 2, 1, 3, 4)
        ...
        output_div_by_energy = torch.sum(
            output_div_by_energy, dim=2
        )  # B x Cout x Hout x Wout
        if debug:
...
class SESConv_Z2_H(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.weight = nn.Parameter(
            torch.Tensor(out_channels, in_channels, self.num_funcs)
        )
        if bias:
    ...
    def forward(self, x):
        ...
        y = F.conv2d(
            x, kernel, bias=None, stride=self.stride, dilation=self.dilation
        )
        B, C, H, W = y.shape
...
class SESConv_H_H(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.weight = nn.Parameter(
            torch.Tensor(out_channels, in_channels, scale_size, self.num_funcs)
        )
        if bias:
            self.bias = nn.Parameter(torch.Tensor(out_channels))
        else:
    ...
    def forward(self, x):
        ...
        for i in range(self.scale_size):
            ...
            output += F.conv2d(
                x_, kernel[:, :, i], groups=S, stride=self.stride
            )
...
class SESConv_H_H_1x1(nn.Module):
    def __init__(
        ...
        if scale_size > 1:
            ...
            self.weight = nn.Parameter(
                torch.Tensor(out_channels, in_channels, scale_size, 1, 1)
            )
        else:
            self.weight = nn.Parameter(
    ...
    def forward(self, x):
        ...
        return F.conv3d(x, weight, padding=[pad, 0, 0], stride=self.stride)[
            :, :, pad:
        ]

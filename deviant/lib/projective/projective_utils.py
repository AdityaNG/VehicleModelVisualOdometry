...
from deviant.lib.backbones.dla import Tree
from deviant.lib.projective.log_polar_conv import (
    DilatedConvolution,
    LogPolarConvolution,
)
from deviant.lib.projective.ses_conv import *
from deviant.lib.projective.sesn_utils import calculate_new_weights
...
def add_SESMax_Scale_to_child(network, child_name):
    ...
    module_list_with_key = OrderedDict(
        [("extra0", getattr(network, child_name)), ("extra1", SESMax_Scale())]
    )
...
def add_SESCopy_Scale_to_child(network, child_name, scales):
    module_list_with_key = OrderedDict(
        [
            ("extra0", SESCopy_Scale(scales=scales)),
            ("extra1", getattr(network, child_name)),
        ]
    )
    new_layer = nn.Sequential(module_list_with_key)
...
def sandwich_layer_between_SESCopy_Scale_and_SESMax_Scale(
    network, child_name, scales
):
    module_list_with_key = OrderedDict(
        [
            ("extra0", SESCopy_Scale(scales=scales)),
...
def replace_all_layers_by_sesn_layers(
    network, scales, replace_style=None, sesn_padding_mode="replicate"
):
    # Reference
    # https://discuss.pytorch.org/t/how-to-replace-all-relu-activations-in-a-pretrained-network/31591/7
    max_scale_before_after_transition = (
        replace_style == "max_scale_before_after_transition"
    )
    max_scale_after_block = replace_style == "max_scale_after_block"
    max_scale_after_block_with_HH = (
        replace_style == "max_scale_after_block_with_HH"
    )
    max_scale_after_conv_kernel_more_than_1 = (
        replace_style == "max_scale_after_conv_kernel_more_than_1"
    )
    max_scale_after_conv_kernel_all = (
        replace_style == "max_scale_after_conv_kernel_all"
    )
    max_scale_after_conv_kernel_all_by_ZH = (
        replace_style == "max_scale_after_conv_kernel_all_by_ZH"
    )
    max_scale_after_dla34_layer = (
        replace_style == "max_scale_after_dla34_layer"
    )

    valid_block_flag = isinstance(network, BasicBlock) or isinstance(
        network, _DenseLayer
    )
    ...
    for child_name, child in network.named_children():
        if max_scale_after_conv_kernel_more_than_1:
            if (
                isinstance(child, nn.MaxPool2d)
                or isinstance(child, nn.AvgPool2d)
                or isinstance(child, nn.BatchNorm2d)
            ):
                ...
                if child.kernel_size[0] > 1:
                    replace_single_layer(
                        network,
                        child,
                        child_name,
                        scales,
                        first_conv=True,
                        sesn_padding_mode=sesn_padding_mode,
                    )
                    network = add_SESMax_Scale_to_child(network, child_name)
                else:
                    pass
            else:
                replace_all_layers_by_sesn_layers(
                    child,
                    scales,
            ...
            if (
                ...
                if effective_size > 1:
                    replace_first_conv_by_1x1_h = False
                    _ = replace_single_layer(
                        network,
                        child,
                        child_name,
                        scales,
                        first_conv=True,
                        replace_first_conv_by_1x1_h=replace_first_conv_by_1x1_h,
                        sesn_padding_mode=sesn_padding_mode,
                    )
                    network = add_SESMax_Scale_to_child(network, child_name)
                else:
                    ...
                    new_layer = nn.Sequential(
                        SESCopy_Scale(scales=scales),
                        getattr(network, child_name),
                        SESMax_Scale(),
                    )
                    setattr(network, child_name, new_layer)
            ...
            if replace_single_layer(
                ...
                if first_conv_flag:
                    ...
                    new_layer = nn.Sequential(
                        SESCopy_Scale(scales=scales), temp_layer
                    )
                    setattr(network, child_name, new_layer)
            else:
                replace_all_layers_by_sesn_layers(
            ...
            if not replace_single_layer(
                network,
                child,
                child_name,
                scales,
                first_conv=first_conv_flag,
                sesn_padding_mode=sesn_padding_mode,
            ):
                replace_all_layers_by_sesn_layers(
                    child,
            ...
            if "norm5" in child_name:
                # Put SESMax_Scale
                new_layer = nn.Sequential(
                    SESMax_Scale(), getattr(network, child_name)
                )
                setattr(network, child_name, new_layer)
                continue
            ...
            if (
                max_scale_after_block
                and "transition" in child_name
                or "norm5" in child_name
            ):
                # Do not replace anything for transition blocks
                continue
...
def add_log_polar_conv(
    network, do_upsample=True, upsample_factor=4, debug=False
):
    # Reference
    # https://discuss.pytorch.org/t/how-to-replace-all-relu-activations-in-a-pretrained-network/31591/7
    for child_name, child in network.named_children():
        if isinstance(child, nn.Conv2d):
            ...
            add_log_polar_conv(
                child, do_upsample=do_upsample, upsample_factor=upsample_factor
            )
...
def replace_and_initialize_with_transform_weights(
    base,
    first_child_name,
    sesn_scales,
    replace_all=True,
    replace_layer_names=None,
    replace_style=None,
    sesn_padding_mode="replicate",
    ...
    if replace_all:
        if "ResNet" in type(base).__name__:
            ...
            for child_name, child in dest.named_children():
                if child_name == "conv1":
                    ...
                    if (
                        child_name == "layer2"
                        or child_name == "layer3"
                        or child_name == "layer4"
                    ):
            ...
            for child_name, child in dest.named_children():
                if (
                    "base_layer" in child_name
                    or "level0" in child_name
                    or "level1" in child_name
                ):
                    replace_single_layer(
                        child,
                        getattr(child, "0"),
                ...
                if "level2" in child_name or "level5" in child_name:
                    ...
                    sandwich_layer_between_SESCopy_Scale_and_SESMax_Scale(
                        dest, child_name, scales=sesn_scales
                    )
                    continue
                ...
                if "level3" in child_name or "level4" in child_name:
                    ...
                    add_SESCopy_Scale_to_child(
                        child, "downsample", scales=sesn_scales
                    )
                    ...
                    replace_single_layer(
                        temp_parent,
                        getattr(temp_parent, "downsample"),
                        "downsample",
                        scales=sesn_scales,
                        first_conv=False,
                        replace_first_conv_by_1x1_h=False,
                    ...
                    sandwich_layer_between_SESCopy_Scale_and_SESMax_Scale(
                        child, "tree1", scales=sesn_scales
                    )
                    ...
                    sandwich_layer_between_SESCopy_Scale_and_SESMax_Scale(
                        child, "tree2", scales=sesn_scales
                    )
            ...
            for child_name, child in dest.named_children():
                ...
                if "level2" in child_name or "level5" in child_name:
                    ...
                    sandwich_layer_between_SESCopy_Scale_and_SESMax_Scale(
                        temp_parent, "0", scales=sesn_scales
                    )
                    ...
                    sandwich_layer_between_SESCopy_Scale_and_SESMax_Scale(
                        temp_parent, "conv1", scales=sesn_scales
                    )
                    replace_single_layer(
                        temp_parent,
                        getattr(temp_parent, "conv2"),
                    ...
                    sandwich_layer_between_SESCopy_Scale_and_SESMax_Scale(
                        temp_parent, "conv2", scales=sesn_scales
                    )
                    ...
                    sandwich_layer_between_SESCopy_Scale_and_SESMax_Scale(
                        temp_parent, "conv", scales=sesn_scales
                    )
        ...
        logging.info(
            "Replacing few layers --> " + ",".join(replace_layer_names)
        )
        num_replace_layers = len(replace_layer_names)

        if (
            max_scale_after_conv_kernel_all
            or max_scale_after_conv_kernel_all_by_ZH
        ):
            # Replace other layers
            for i in range(num_replace_layers):
                ...
                if curr_layer_name not in all_layer_names:
                    logging.info(
                        "WARNING! replace_layer_name {} not present in the network. Exiting!!!".format(
                            curr_layer_name
                        )
                    )
                    continue
                ...
                if curr_layer_name == first_child_name:
                    ...
                    _ = replace_single_layer(
                        network=dest,
                        child=getattr(dest, first_child_name),
                    ...
                    if (
                        max_scale_after_conv_kernel_more_than_1
                        or max_scale_after_conv_kernel_all
                    ):
                        dest = add_SESMax_Scale_to_child(
                            dest, first_child_name
                        )

                # Replace other layers
                replace_all_layers_by_sesn_layers(
                    getattr(dest, curr_layer_name),
                    scales=sesn_scales,
                    replace_style=replace_style,
                    sesn_padding_mode=sesn_padding_mode,
                )
            ...
            for i in range(num_replace_layers):
                ...
                if curr_layer_name not in all_layer_names:
                    ...
                    continue
                curr_layer_index_in_object = all_layer_names.index(
                    curr_layer_name
                )

                if max_scale_after_block or max_scale_after_block_with_HH:
                    if (
                        "transition" in curr_layer_name
                        or "norm5" in curr_layer_name
                    ):
                        # Do not replace anything for transition blocks
                        continue

                if not (
                    max_scale_after_block or max_scale_after_block_with_HH
                ):
                    if i == 0 and replace_layer_names[i] != all_layer_names[0]:
                        # Add an extra SESCopy_Scale before the first layer
                        prev_layer_name = all_layer_names[
                            curr_layer_index_in_object - 1
                        ]
                        new_conv = SESCopy_Scale(scales=sesn_scales)

                        layer_list_with_key = OrderedDict(
                            [
                                ("orig", getattr(dest, prev_layer_name)),
                                ("extra", new_conv),
                            ]
                        )
                ...
                if curr_layer_name == first_child_name:
                    ...
                    curr_layer_name == "norm0"
                    or curr_layer_name == "norm1"
                    or curr_layer_name == "norm5"
                ):
                    # Instance of single layer
                    _ = replace_single_layer(
                        network=dest,
                        child=getattr(dest, curr_layer_name),
                        child_name=curr_layer_name,
                        scales=sesn_scales,
                        sesn_padding_mode=sesn_padding_mode,
                    )
                    ...
                    if max_scale_after_block or max_scale_after_block_with_HH:
                        new_layer = nn.Sequential(
                            getattr(dest, pool_layer), SESMax_Scale()
                        )
                        setattr(dest, pool_layer, new_layer)
    ...
    for index_for_init in range(len(sesn_scales)):
        dest_state_dict = calculate_new_weights(
            src_input=base,
            dest_input=dest,
    ...
    if (
        max_scale_before_after_transition
        or max_scale_after_block
        or max_scale_after_block_with_HH
        or max_scale_after_conv_kernel_more_than_1
        or max_scale_after_conv_kernel_all
        or max_scale_after_dla34_layer
        or max_scale_after_conv_kernel_all_by_ZH
    ):

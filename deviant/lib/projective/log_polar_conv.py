...
def get_max_coordinate(center_dim, max_of_this_dim):
    if type(center_dim) == torch.Tensor:
        max_coordinate = torch.max(
            torch.cat(
                [
                    center_dim.unsqueeze(0),
                    max_of_this_dim - center_dim.unsqueeze(0),
                ]
            )
        )
...
def index_4D_tensor_by_two_coordinates(input, index_x, index_y):
    ...
    if index_x.dim() == 2:
        ...
        index_x = (
            index_x.unsqueeze(0).unsqueeze(0).repeat(batch, channel, 1, 1)
        )
        index_y = (
            index_y.unsqueeze(0).unsqueeze(0).repeat(batch, channel, 1, 1)
        )
    ...
    index_x = index_x.reshape(
        batch * channel, -1
    )  # batch*channel x grid_h*grid_w
    index_y = index_y.reshape(
        batch * channel, -1
    )  # batch*channel x grid_h*grid_w

    # Get illegal indices outside the range
    invalid_mask_for_index_x = logical_or(
        index_x < 0, index_x >= w
    )  # batch*channel x grid_h*grid_w
    invalid_mask_for_index_y = logical_or(
        index_y < 0, index_y >= h
    )  # batch*channel x grid_h*grid_w
    invalid_mask = logical_or(
        invalid_mask_for_index_x, invalid_mask_for_index_y
    )  # batch*channel x grid_h*grid_w
    ...
    output_2d = torch.gather(
        input_2d, dim=1, index=index_2d
    )  # batch*channel x grid_h*grid_w
...
def interpolate_feature_map(
    input_feature_map, grid_normalized, mode="bilinear"
):
    ...
    grid_normalized = cast_to_cpu_cuda_tensor(
        grid_normalized, input_feature_map
    )
    ...
    grid_x = grid_x.unsqueeze(1).repeat(
        1, channel, 1, 1
    )  # batch x channel x grid_h x grid_w
    grid_y = grid_y.unsqueeze(1).repeat(
        1, channel, 1, 1
    )  # batch x channel x grid_h x grid_w
    ...
    top_left_pixel = index_4D_tensor_by_two_coordinates(
        input=input_feature_map, index_x=left, index_y=top
    )  # batch x channel x grid_h x grid_w
    top_right_pixel = index_4D_tensor_by_two_coordinates(
        input=input_feature_map, index_x=right, index_y=top
    )  # batch x channel x grid_h x grid_w
    bottom_left_pixel = index_4D_tensor_by_two_coordinates(
        input=input_feature_map, index_x=left, index_y=bottom
    )  # batch x channel x grid_h x grid_w
    bottom_right_pixel = index_4D_tensor_by_two_coordinates(
        input=input_feature_map, index_x=right, index_y=bottom
    )  # batch x channel x grid_h x grid_w

    dist_left = grid_x - left.type(
        input_feature_map.dtype
    )  # batch x channel x grid_h x grid_w
    dist_right = (
        right.type(input_feature_map.dtype) - grid_x
    )  # batch x channel x grid_h x grid_w
    dist_top = grid_y - top.type(
        input_feature_map.dtype
    )  # batch x channel x grid_h x grid_w
    dist_bottom = (
        bottom.type(input_feature_map.dtype) - grid_y
    )  # batch x channel x grid_h x grid_w
    ...
    if mode == "bilinear":
        ...
        output = (
            dist_bottom * dist_right * top_left_pixel
...
def cartesian_to_log_polar_feature_map(
    input_feature_map,
    center_cartesian_feature_map=None,
    padding_mode="zeros",
    interpolation_method="deterministic",
    upsample_factor_for_log_polar_output=1.0,
):
    """
    ...
    if type(input_feature_map) == torch.Tensor:
        ...
        center_x, center_y = get_centers(
            center_cartesian_feature_map, input_feature_map
        )
        ...
        log_r_vector = (
            torch.arange(out_w).type(input_feature_map.dtype) / log_r_upsamp
        )
        theta_vector = (
            torch.arange(out_h).type(input_feature_map.dtype) / theta_upsamp
        )
        ...
        x_grid, y_grid = polar_to_cartesian(
            R=r_grid, Theta=theta_grid
        )  # out_h x out_w

        # Centered at top-left
        x_grid_top_center = x_grid.unsqueeze(0).repeat(
            batch, 1, 1
        ) + center_x.unsqueeze(1).unsqueeze(
            1
        )  # batch x out_h x out_w
        y_grid_top_center = y_grid.unsqueeze(0).repeat(
            batch, 1, 1
        ) + center_y.unsqueeze(1).unsqueeze(
            1
        )  # batch x out_h x out_w

        # Bring x_grid_normalized and y_grid_normalized in the range [-1, 1]
        x_grid_normalized = (
            x_grid_top_center * 2.0 / w - 1.0
        )  # batch x out_h x out_w
        y_grid_normalized = (
            y_grid_top_center * 2.0 / h - 1.0
        )  # batch x out_h x out_w
        ...
        #  V
        #  Y
        grid_final = torch.cat(
            (x_grid_normalized.unsqueeze(3), y_grid_normalized.unsqueeze(3)),
            dim=3,
        )  # batch x out_h x out_w x 2
        grid_final = grid_final.detach()

        # Log Polar coordinate sample
        if interpolation_method == "deterministic":
            log_polar_feature_map = interpolate_feature_map(
                input_feature_map, grid_normalized=grid_final
            )
        else:
            log_polar_feature_map = F.grid_sample(
                input_feature_map,
                grid_final,
                mode="bilinear",
        ...
        log_polar_feature_map = cv2.logPolar(
            input_feature_map,
            center=(center_x, center_y),
            M=min(center_x, center_y),
            flags=0,
        )
...
def log_polar_to_cartesian_feature_map(
    log_polar_feature_map,
    center_cartesian_feature_map=None,
    padding_mode="zeros",
    interpolation_method="deterministic",
    upsample_factor_for_log_polar_output=1.0,
    ...
    if type(log_polar_feature_map) == torch.Tensor:
        ...
        temp_feat_map = torch.zeros((batch, 1, h_out, w_out)).type(
            log_polar_feature_map.dtype
        )
        center_x, center_y = get_centers(
            center_cartesian_feature_map, input_feature_map=temp_feat_map
        )

        x_max_out = get_max_coordinate(
            center_dim=center_x, max_of_this_dim=w_out
        ).float()
        y_max_out = get_max_coordinate(
            center_dim=center_y, max_of_this_dim=h_out
        ).float()
        r_max_out = torch.sqrt(x_max_out * x_max_out + y_max_out * y_max_out)

        log_r_max = torch.log(
            r_max_out * upsample_factor_for_log_polar_output + EPS
        )
        theta_max = 2.0 * PI
        ...
        # Make x, y grid now
        x_grid_out = x_vector_out.unsqueeze(0).unsqueeze(0).repeat(
            batch, y_vector_out.shape[0], 1
        ) - center_x.unsqueeze(1).unsqueeze(
            1
        )  # batch x h_out x w_out  each entry in [0, h_out]
        y_grid_out = y_vector_out.unsqueeze(1).unsqueeze(0).repeat(
            batch, 1, x_vector_out.shape[0]
        ) - center_y.unsqueeze(1).unsqueeze(
            1
        )  # batch x h_out x w_out  each entry in [0, h_out]
        ...
        x_grid = (
            x_grid_out * upsample_factor_for_log_polar_output
        )  # batch x h_out x w_out
        y_grid = (
            y_grid_out * upsample_factor_for_log_polar_output
        )  # batch x h_out x w_out
        ...
        log_r_grid_top_center = (
            log_r_grid * log_r_upsamp
        )  # batch x h_out x w_out
        theta_grid_top_center = (
            theta_grid * theta_upsamp
        )  # batch x h_out x w_out

        # Bring log_r_grid and theta_grid in the range [-1, 1]
        log_r_grid_normalized = (
            log_r_grid_top_center * 2.0 / w - 1.0
        )  # batch x h_out x w_out
        theta_grid_normalized = (
            theta_grid_top_center * 2.0 / h - 1.0
        )  # batch x h_out x w_out
        ...
        grid_final = torch.cat(
            (
                log_r_grid_normalized.unsqueeze(3),
                theta_grid_normalized.unsqueeze(3),
            ),
            dim=3,
        )  # batch x h_out x w_out x 2
        grid_final = grid_final.detach()

        # Inverse Log polar sample
        if interpolation_method == "deterministic":
            cartesian_feature_map = interpolate_feature_map(
                log_polar_feature_map, grid_normalized=grid_final
            )
        else:
            cartesian_feature_map = F.grid_sample(
                log_polar_feature_map,
                grid_final,
                mode="bilinear",
                padding_mode=padding_mode,
        ...
        # Get the center first
        center_x, center_y = get_centers(
            center_cartesian_feature_map,
            input_feature_map=log_polar_feature_map,
        )

        # CHW --> HWC
        log_polar_feature_map = convert_CHW_to_HWC(log_polar_feature_map)

        cartesian_feature_map = cv2.logPolar(
            log_polar_feature_map,
            center=(center_x, center_y),
            M=min(center_x, center_y),
        ...
        for i in range(self.num_scales):
            ...
            if i == 0:
                ...
                max_output = torch.max(
                    torch.stack((max_output, dil_output)), dim=0
                )[0]
...
class LogPolarConvolution(torch.nn.Module):
    ...
    def forward(self, input, center_input=None):
        if self.do_upsample and self.upsample_factor != 1:
            ...
            input_upsamp = our_interpolate(
                input, scale_factor=self.upsample_factor
            )
        ...
        log_polar_input_upsamp = cartesian_to_log_polar_feature_map(
            input_upsamp,
            center_cartesian_feature_map=center_upsamp,
            interpolation_method="pytorch",
        )
        log_polar_output = self.conv(log_polar_input_upsamp)
        output_upsamp = log_polar_to_cartesian_feature_map(
            log_polar_output,
            center_cartesian_feature_map=center_upsamp,
            interpolation_method="pytorch",
        )
        ...
        if self.do_upsample and self.upsample_factor != 1:
            ...
            output = our_interpolate(
                output_upsamp, scale_factor=1.0 / self.upsample_factor
            )
        else:

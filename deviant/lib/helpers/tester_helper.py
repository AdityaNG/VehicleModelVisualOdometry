...
from deviant.lib.helpers.save_helper import load_checkpoint


class Tester(object):
    def __init__(
        self, cfg, model, data_loader, logger, results_test="result_test"
    ):
        self.cfg = cfg
        self.model = model
        ...
        self.eval_dataset = (
            ...
            if "eval_dataset" in cfg["dataset"].keys()
            else cfg["dataset"]["type"]
        )
        ...
        self.get_backbone_features = (
            False
            if "get_backbone_features" not in self.cfg["tester"].keys()
            else self.cfg["tester"]["get_backbone_features"]
        )
    ...
    def test(self):
        ...
        for batch_idx, (inputs, calibs, coord_ranges, _, info) in enumerate(
            self.data_loader
        ):
            ...
            outputs = self.model(
                inputs, coord_ranges, calibs, K=50, mode="test"
            )
            ...
            info = {
                key: val.detach().cpu().numpy() for key, val in info.items()
            }
        ...
        if self.get_backbone_features:
            ...
            for level in range(len(results)):
                ...
                save_numpy(
                    os.path.join(
                        npy_folder_name, "level_" + str(level) + ".npy"
                    ),
                    results[level],
                )
            return

...
def build_lr_scheduler(cfg, optimizer, last_epoch):
    ...
    if cfg["warmup"]:
        warmup_lr_scheduler = CosineWarmupLR(
            optimizer, num_epoch=5, init_lr=0.00001
        )
...
if __name__ == "__main__":
    ...
    from deviant.lib.models.centernet3d import CenterNet3D

    net = CenterNet3D()
    optimizer = optim.Adam(net.parameters(), 0.01)
    lr_warmup_scheduler_cosine = CosineWarmupLR(
        optimizer, 1000, init_lr=0.00001, last_epoch=-1
    )
    lr_warmup_scheduler_linear = LinearWarmupLR(
        optimizer, 1000, init_lr=0.00001, last_epoch=-1
    )

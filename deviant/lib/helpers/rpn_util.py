...
from deviant.lib.helpers.file_io import *
from deviant.lib.helpers.util import *


def evaluate_nusc_kitti_results_verbose(
    results_folder, conf=None, use_logging=False, logger=None
):
    ...
    for i in range(len(val_org_samples)):
        ...
        intended_path = os.path.join(
            results_folder_new, val_org_samples[i] + ".txt"
        )
        os.symlink(
            os.path.join(os.getcwd(), src_path),
            os.path.join(os.getcwd(), intended_path),
        )
    ...
    home_word_index = [
        idx for idx, s in enumerate(curr_folder) if "home" in s
    ][0]
    home_user_folder = "/".join(curr_folder[: (home_word_index + 2)])
    python_path = os.path.join(
        home_user_folder, "anaconda3/envs/nuscenes/bin/python"
    )  # /home/abc/anaconda3/envs/nuscenes/

    # path of the nuscenes-devkit
    devkit_path = os.path.join(
        os.getcwd(), "nuscenes-devkit/python-sdk/"
    )  # /home/abc/project/DEVIANT/code/nuscenes-devkit/python-sdk
    devkit_path_more = os.path.join(
        devkit_path, "nuscenes"
    )  # /home/abc/project/DEVIANT/code/nuscenes-devkit/python-sdk/nuscenes
    ...
    command_convert = (
        python_path
        + " "
        + os.path.join(devkit_path_more, "scripts/export_kitti.py")
        + " kitti_res_to_nuscenes --nusc_version v1.0-trainval --split val --image_count 6019 "
        + " --nusc_kitti_dir "
        + "data/nusc_kitti/nusc_kitti_org"
        + " --output_dir "
        + results_folder_new
    )
    ...
    command_evaluate = (
        python_path
        + " "
        + os.path.join(devkit_path_more, "eval/detection/evaluate.py")
    ...
    if use_logging:
        ...
        try:
            ...
            safe_subprocess(
                "mv "
                + os.path.join(results_folder_new, "submission.json")
                + " "
                + output_folder
            )
            ...
            logger.info(
                "An exception occurred while nusc_kitti evaluation ..."
            )
    else:
...
def get_MAE(
    ...
    if use_logging:
        logger.info(
            "Found {} predictions in {}, GT= {}".format(
                num_images, results_folder, gt_folder
            )
        )
    else:
        print(
    ...
    for i in range(num_images):
        ...
        gt_boxes_orig = read_csv(
            path=gt_file, delimiter=" ", ignore_warnings=False, use_pandas=True
        )
        ...
        for j in range(1, thresholds.shape[0]):
            valid_gt_index_for_th = np.logical_and(
                gt_boxes >= thresholds[j - 1], gt_boxes < thresholds[j]
            )
        ...
        if num_valid_boxes > 0:
            ...
            # All boxes data
            error_box[0] = custom_append(
                storage=error_box[0], curr=error_boxes_valid
            )
            pred_box_cnt[0] += num_valid_boxes

            # GT binned threshold boxes data
            for j in range(1, thresholds.shape[0]):
                valid_index_for_th = np.logical_and(
                    gt_z_valid >= thresholds[j - 1], gt_z_valid < thresholds[j]
                )
                num_valid_boxes_th = np.sum(valid_index_for_th)

                if num_valid_boxes_th > 0:
                    error_box[j] = custom_append(
                        storage=error_box[j],
                        curr=error_boxes_valid[valid_index_for_th],
                    )
...
def evaluate_waymo_results_verbose(
    results_folder, conf=None, use_logging=False, logger=None
):
    # Find home in path
    ...
    path = os.path.join(
        home_user_folder, "anaconda3/envs/py36_waymo_tf/bin/python"
    )  # /home/abc/anaconda3/envs/py36_waymo_tf/
    pd_set = os.path.join(
        "data/waymo/ImageSets", conf["dataset"]["val_split_name"] + ".txt"
    )

    command_0_7 = (
        path
        + " -u data/waymo/waymo_eval.py --predictions "
        + results_folder
        + " --pd_set "
        + pd_set
    )
    command_0_5 = (
        path
        + " -u data/waymo/waymo_eval_0_5.py --predictions "
        + results_folder
        + " --pd_set "
        + pd_set
    )
    ...
    if use_logging:
        ...
        logger.info(
            subprocess.check_output(command_0_7, shell=True, text=True)
        )
        logger.info("Running for thresholds [0.5, 0.3, 0.3]...")
        logger.info(
            subprocess.check_output(command_0_5, shell=True, text=True)
        )
    else:
...
def evaluate_kitti_results_verbose(
    data_folder,
    test_dataset_name,
    results_folder,
    ...
    binary_test_dataset_name = (
        "kitti_split1"  # test_dataset_name   # Use binaries of kitti_split1
    )
    eval_binary_path = os.path.join(
        devkit_folder, binary_test_dataset_name, "devkit", "cpp", default_eval
    )
    ...
    results_obj.main = run_kitti_eval_script(
        eval_binary_path,
        results_data=stats_save_folder,
        gt_folder=gt_folder,
        lbls=lbls,
        use_40=True,
    )
    ...
    for lbl in lbls:
        ...
        for task in task_keys:
            ...
            if task_lbl in results_obj.main:
                ...
                if task_print == "gr":
                    task_print = "bev"
                print_str = "test_iter {} {} {:3s} --> easy: {:0.4f}, mod: {:0.4f}, hard: {:0.4f}".format(
                    test_iter, lbl, task_print, easy, mod, hard
                )
    ...
    results_obj.side = run_kitti_eval_script(
        eval_binary_path,
        results_data=stats_save_folder,
        gt_folder=gt_folder,
        lbls=lbls,
    ...
    for k, dis_key in enumerate(dis_keys):
        if k == 0:
            print(
                "Getting AP3D at        ground truth distance <= {}m".format(
                    dis_key
                ),
                end="",
                flush=True,
            ...
            print(
                "Getting AP3D at {}m <= ground truth distance <= {}m".format(
                    dis_keys[k - 1], dis_key
                ),
                end="",
                flush=True,
            )
    ...
    pickle_write(
        os.path.join(
            stats_save_folder,
            "AP_vs_IOU3D_threshold_at_different_gt_distances.pkl",
        ),
        results_obj,
    )


def run_kitti_eval_script(
    eval_binary_path, results_data, gt_folder, lbls, use_40=True
):
    # evaluate primary experiment
    with open(os.devnull, "w") as devnull:
        _ = subprocess.check_output(
            [eval_binary_path, results_data, gt_folder], stderr=devnull
        )
    ...
    for lbl in lbls:
        ...
        respath_2d = os.path.join(
            results_data, "stats_{}_detection.txt".format(lbl)
        )
        respath_or = os.path.join(
            results_data, "stats_{}_orientation.txt".format(lbl)
        )
        respath_gr = os.path.join(
            results_data, "stats_{}_detection_ground.txt".format(lbl)
        )
        respath_3d = os.path.join(
            results_data, "stats_{}_detection_3d.txt".format(lbl)
        )
...
def intersect(box_a, box_b, mode="combinations", data_type=None):
    ...
    if mode == "combinations":
        ...
        if data_type == np.ndarray:
            ...
            max_xy = np.minimum(
                box_a[:, 2:4], np.expand_dims(box_b[:, 2:4], axis=1)
            )
            min_xy = np.maximum(
                box_a[:, 0:2], np.expand_dims(box_b[:, 0:2], axis=1)
            )
...
def iou(box_a, box_b, mode="combinations", data_type=None):
    ...
    if mode == "combinations":
        ...
        # np.ndarray
            ...
            union = (
                np.expand_dims(area_a, 0) + np.expand_dims(area_b, 1) - inter
            )

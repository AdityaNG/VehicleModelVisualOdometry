...
def write_csv(
    path, numpy_variable, delimiter=" ", save_folder=None, float_format=None
):
    if save_folder is not None:
        path = os.path.join(save_folder, path)

    pd.DataFrame(numpy_variable).to_csv(
        path, sep=delimiter, header=None, index=None, float_format=float_format
    )

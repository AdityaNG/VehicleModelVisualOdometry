...
from deviant.lib.helpers.decode_helper import (
    decode_detections,
    extract_dets_from_outputs,
)
from deviant.lib.helpers.rpn_util import *
from deviant.lib.helpers.save_helper import (
    get_checkpoint_state,
    load_checkpoint,
    save_checkpoint,
)
from deviant.lib.losses.loss_function import (
    GupnetLoss,
...
class Trainer(object):
    def __init__(
        ...
        self.tester_metrics = (
            "kitti"
            if "tester_metrics" not in self.cfg_test.keys()
            else self.cfg_test["tester_metrics"]
        )
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.cfg_train["max_epoch"]):
            ...
            if self.warmup_lr_scheduler is not None and epoch < 5:
                ...
                self.logger.info(
                    "Learning Rate: %f" % self.lr_scheduler.get_lr()[0]
                )
            ...
            # save trained model
            if (
                self.epoch % self.cfg_train["save_frequency"] == 0
                or self.epoch == self.cfg_train["max_epoch"]
            ):
                os.makedirs(
                    self.cfg_train["log_dir"] + "/checkpoints", exist_ok=True
                )
            ...
            if (
                self.epoch % self.cfg_train["eval_frequency"] == 0
                or self.epoch == self.cfg_train["max_epoch"]
            ):
                self.logger.info(
                    "------ EVAL EPOCH %03d ------" % (self.epoch)
                )
                self.eval_one_epoch()
    ...
    def train_one_epoch(self, loss_weights=None):
        ...
        for batch_idx, (
            ...
            if trained_batch % self.cfg_train[
                "disp_frequency"
            ] == 0 or trained_batch == len(self.train_loader):
                log_str = "BATCH[%04d/%04d]" % (
                    trained_batch,
    ...
    def eval_one_epoch(self):
        ...
        with torch.no_grad():
            for batch_idx, (
                ...
                outputs = self.model(
                    inputs, coord_ranges, calibs, K=50, mode="val"
                )
        ...
        output_folder = os.path.join(
            self.cfg_train["log_dir"], "result_" + str(self.epoch)
        )
        ...
        if self.tester_metrics == "kitti":
            evaluate_kitti_results_verbose(
                data_folder="data/KITTI/training/label_2",
                test_dataset_name="val1",
                results_folder=results_folder,
                test_iter=self.epoch,
                conf=self.cfg,
                use_logging=True,
                logger=self.logger,
            )
        elif self.tester_metrics == "waymo":
            evaluate_waymo_results_verbose(
                results_folder=results_folder,
                conf=self.cfg,
                use_logging=True,
                logger=self.logger,
            ...
            evaluate_nusc_kitti_results_verbose(
                results_folder=results_folder,
                conf=self.cfg,
                use_logging=True,
                logger=self.logger,

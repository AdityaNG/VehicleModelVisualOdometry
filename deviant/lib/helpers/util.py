...
from deviant.lib.helpers.file_io import read_csv
...
def draw_text(
    ...
    if bg_color is not None:
        ...
        x_e = int(
            np.clip(pos[0] + text_size[0] - 1 + 4, a_min=0, a_max=im.shape[1])
        )
        y_s = int(
            np.clip(pos[1] - text_size[1] - 2, a_min=0, a_max=im.shape[0])
        )
...
def get_downsampled_image(img, curr_scale, height_first_flag=True):
    if height_first_flag:
        ...
        new_img_pil = img_pil.resize(
            (new_shape[1], new_shape[0]), Image.ANTIALIAS
        )
        new_img = cv2.cvtColor(np.array(new_img_pil), cv2.COLOR_RGB2BGR)

        # Paste the input image
        left_x, right_x, left_y, right_y = get_bounds(
            img_shape_x=img.shape[0],
            img_shape_y=img.shape[1],
            new_shape_x=new_shape[0],
            new_shape_y=new_shape[1],
        )
        ...
        new_img_temp = resize(
            img_temp, (new_shape[1], new_shape[2], new_shape[0])
        )  # HWC
...
def is_slanted(label_file, ego_height=1.65):
    ...
    if gt_img is not None:
        ...
        if np.sum(car_index) > 0:
            ...
            good_cars_count = np.sum(
                np.logical_and(
                    cars_filter_y3d >= 0.0, cars_filter_y3d <= ego_height + 1.0
                )
            )

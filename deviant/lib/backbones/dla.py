...
class Tree(nn.Module):
    ...
    def forward(self, x_input):
        ...
        # from deviant.lib.projective.projective_utils import get_child_names_of_network
        ...
        if self.levels == 1:
            ...
            if len(children) > 0 and children[0].dim() == 5 and x1.dim() == 4:
                x1_cloned = x1.unsqueeze(2).repeat(
                    1, 1, children[0].shape[2], 1, 1
                )
            else:
                x1_cloned = x1
            ...
            if isinstance(self.tree2, BasicBlock) or isinstance(
                self.tree2, Bottleneck
            ):

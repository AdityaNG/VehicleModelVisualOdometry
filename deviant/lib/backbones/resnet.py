...
from deviant.lib.projective.ses_conv import ses_max_projection
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, **kwargs):
        ...
        self.return_levels = (
            True if "return_levels" not in kwargs else kwargs["return_levels"]
        )
        self.in_planes = 64
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        for i in range(1, blocks):
            layers.append(
                block(self.in_planes, planes, stride=1, downsample=None)
            )

...
from deviant.lib.backbones.dla import dla34, dla60, dla102, dla169
from deviant.lib.backbones.dlaup import DLAUp, DLAUpv2
from deviant.lib.backbones.resnet import resnet18, resnet50
from deviant.lib.helpers.decode_helper import _nms, _topk
from deviant.lib.losses.loss_function import extract_input_from_tensor
from deviant.lib.projective.projective_utils import *
...
class GUPNet(nn.Module):
    def __init__(
        ...
        self.get_backbone_features = (
            False
            if "get_backbone_features" not in self.cfg_parent["tester"].keys()
            else self.cfg_parent["tester"]["get_backbone_features"]
        )
        self.backbone = globals()[backbone](
            pretrained=True, return_levels=True
        )
        ...
        self.feat_up = globals()[neck](
            channels[self.first_level :], scales_list=scales
        )

        self.use_conv = (
            "vanilla"
            if "use_conv" not in self.cfg.keys()
            else self.cfg["use_conv"]
        )
        self.sesn_scales = (
            [0.83, 1.0, 1.2]
            if "sesn_scales" not in self.cfg.keys()
            else self.cfg["sesn_scales"]
        )
        self.replace_all = (
            True
            if "replace_all" not in self.cfg.keys()
            else self.cfg["replace_all"]
        )
        self.replace_layer_names = (
            None
            if "replace_layer_names" not in self.cfg.keys()
            else self.cfg["replace_layer_names"]
        )
        self.replace_style = (
            "max_scale_after_dla34_layer"
            if "replace_style" not in self.cfg.keys()
            else self.cfg["replace_style"]
        )
        self.scale_index_for_init = (
            0
            if "scale_index_for_init" not in self.cfg.keys()
            else self.cfg["scale_index_for_init"]
        )
        self.sesn_padding_mode = (
            "constant"
            if "sesn_padding_mode" not in self.cfg.keys()
            else self.cfg["sesn_padding_mode"]
        )

        if self.use_conv == "sesn":
            output_string = "Replace by {} conv... scales= {} | padding= {} | scale_index_for_init= {}".format(
                self.use_conv,
                ",".join([str(t) for t in self.sesn_scales]),
                self.sesn_padding_mode,
                self.scale_index_for_init,
            )
            logging.info(output_string)
            self.backbone = replace_and_initialize_with_transform_weights(
                base=self.backbone,
            ...
            logging.info(output_string)
            add_dilated_conv(
                network=self.backbone, scales=self.sesn_scales, debug=False
            )
    ...
    def forward(
        self, input, coord_ranges, calibs, targets=None, K=50, mode="train"
    ):
        ...
        if (
            ...
            inds, cls_ids = _topk(
                _nms(
                    torch.clamp(
                        ret["heatmap"].sigmoid(), min=1e-4, max=1 - 1e-4
                    )
                ),
                K=K,
            )[1:3]
            masks = (
                torch.ones(inds.size()).type(torch.uint8).to(device_id).bool()
            )
        ret.update(
            self.get_roi_feat(
                feat, inds, masks, ret, calibs, coord_ranges, cls_ids
            )
        )
        return ret

    def get_roi_feat_by_mask(
        self, feat, box2d_maps, inds, mask, calibs, coord_ranges, cls_ids
    ):
        ...
        if num_masked_bin != 0:
            ...
            cls_hots[
                torch.arange(num_masked_bin).to(device_id),
                cls_ids[mask].long(),
            ] = 1.0
            ...
            depth_geo = (
                size_3d[:, 0] / box2d_height.squeeze() * roi_calibs[:, 0, 0]
            )

            depth_net_out = self.depth(roi_feature_masked)[:, :, 0, 0]
            depth_geo_log_std = (
                h3d_log_std.squeeze()
                + 2 * (roi_calibs[:, 0, 0].log() - box2d_height.log())
            ).unsqueeze(-1)
            depth_net_log_std = torch.logsumexp(
                torch.cat([depth_net_out[:, 1:2], depth_geo_log_std], -1),
                -1,
                keepdim=True,
            )
            ...
            res["train_tag"] = (
                torch.ones(num_masked_bin).type(torch.bool).to(device_id)
            )
    ...
    def get_roi_feat(
        self, feat, inds, mask, ret, calibs, coord_ranges, cls_ids
    ):
        BATCH_SIZE, _, HEIGHT, WIDE = feat.size()
        device_id = feat.device
        coord_map = (
        ...
        res = self.get_roi_feat_by_mask(
            feat, box2d_maps, inds, mask, calibs, coord_ranges, cls_ids
        )
        return res

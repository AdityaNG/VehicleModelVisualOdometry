...
def convolution_with_normalized_patches(
    ...
    for i in range(groups):
        ...
        inp_unf = torch.nn.functional.unfold(
            inp_group, (K, K), padding=padding, stride=stride
        )  # B x Cin*K*K X Hout*Wout
        inp_unf_interm = inp_unf.transpose(1, 2).unsqueeze(
            1
        )  # B x 1 x Hout*Wout x Cin*K*K

        norm_kernel_wt_interm = (
            norm_kernel_wt[index_output[i] : index_output[i + 1]]
            .view(out_channels_grp, -1)
            .unsqueeze(0)
            .unsqueeze(2)
        )  # 1 x Cout_grp x 1 x Cin*K*K
        out_unf = torch.mul(inp_unf_interm, norm_kernel_wt_interm).reshape(
            batch,
            out_channels_grp,
            inp_unf_interm.shape[2],
            in_channels_grp,
            -1,
        )  # B x Cout_grp x Hout*Wout x Cin x K*K
        output_grp = (
            torch.sum(out_unf, dim=4)
            .transpose(2, 3)
            .reshape(batch, out_channels_grp, in_channels_grp, Hout, Wout)
        )  # B x Cout_grp x Cin_grp x Hout x Wout
        if debug:
...
out2 = convolution_with_normalized_patches(
    inp,
    kernel_wt=w,
    padding=padding,
    stride=stride,
...
print(
    (
        torch.nn.functional.conv2d(
            inp, w, padding=padding, stride=stride, groups=groups
        )
        - out2
    )
    .abs()

...
from deviant.lib.helpers.file_io import imread
from deviant.lib.projective.projective_utils import LogPolarConvolution
...
def get_pair_constraint_status(output_big, output_small, margin=0.0):
    ...
    if (
        output_big - output_small >= margin
        or torch.abs(output_big) < EPS
        or torch.abs(output_small) < EPS
    ):
        return 1
    else:
        return 0
...
def constraint_satisfied(
    response_small, response_mid, response_big, margin=0.0
):
    """Checks if all the constraints are satisfied"""

    check_arr = np.zeros((6,)).astype(np.uint8)

    check_arr[0] = get_pair_constraint_status(
        output_big=response_small[0],
        output_small=response_small[1],
        margin=margin,
    )
    check_arr[1] = get_pair_constraint_status(
        output_big=response_small[0],
        output_small=response_small[2],
        margin=margin,
    )

    check_arr[2] = get_pair_constraint_status(
        output_big=response_mid[1], output_small=response_mid[0], margin=margin
    )
    check_arr[3] = get_pair_constraint_status(
        output_big=response_mid[1], output_small=response_mid[2], margin=margin
    )

    check_arr[5] = get_pair_constraint_status(
        output_big=response_big[2], output_small=response_big[1], margin=margin
    )
    check_arr[4] = get_pair_constraint_status(
        output_big=response_big[2], output_small=response_big[0], margin=margin
    )
...
def get_response_of_filter_bank(
    basis, show_optimization=True, opt_margin=0.0, check_margin=0.0
):
    if basis.shape[2] == 5:
        response_small_all = basis[:, :, 2, 2]
        response_mid_all = torch.sum(
            torch.sum(basis[:, :, 1:4, 1:4], dim=3), dim=2
        )
        response_big_all = torch.sum(
            torch.sum(basis[:, :, :, :], dim=3), dim=2
        )
    elif basis.shape[2] == 9:
        response_small_all = basis[:, :, 4, 4]
        response_mid_all = torch.sum(
            torch.sum(basis[:, :, 2:7, 2:7], dim=3), dim=2
        )
        response_big_all = torch.sum(
            torch.sum(basis[:, :, :, :], dim=3), dim=2
        )
    ...
    for filter_ind in range(num_filters):
        ...
        check_flag, check_sum = constraint_satisfied(
            response_small, response_mid, response_big, margin=check_margin
        )
...
fig_sim, ((ax1, ax2), (ax3, ax4)) = plt.subplots(
    2, 2, figsize=(params.size[0] * 1.5, params.size[1] * 2), dpi=params.DPI
)

for rescale_index, rescale_basis in enumerate([False, True]):
    for norm_index, norm_per_scale in enumerate([False, True]):
        gif_path_prefix = "images/test_ses_basis_orthogonality_size_" + str(
            effective_size
        )
        ...
        if effective_size == 7:
            ...
            model_ses = nn.Sequential(
                SESCopy_Scale(scales=sesn_scales),
                SESConv_H_H(
                    in_channels=1,
                    out_channels=1,
                    kernel_size=kernel_size,
        ...
        for filter_set_index in range(start_index, num_filter_sets):
            if get_toy_gif:
                ...
                img_single_ch_np = (
                    model_ses[ses_layer_index]
                    .basis[filter_set_index, 0]
                    .detach()
                    .numpy()
                )
                img_scaled_single_ch_np = (
                    model_ses[ses_layer_index]
                    .basis[filter_set_index, 1]
                    .detach()
                    .numpy()
                )
                if len(sesn_scales) > 2:
                    img_scaled_single_ch_np2 = (
                        model_ses[ses_layer_index]
                        .basis[filter_set_index, 2]
                        .detach()
                        .numpy()
                    )
            ...
            img_tensor = convert_to_tensor(
                img_single_ch_np, normalize=normalize
            )
            img_scaled_tensor = convert_to_tensor(
                img_scaled_single_ch_np, normalize=normalize
            )
            if len(sesn_scales) > 2:
                img_scaled_tensor2 = convert_to_tensor(
                    img_scaled_single_ch_np2, normalize=normalize
                )

            if ses_layer_index == 0:
                our_weight = torch.zeros(
                    (1, 1, effective_size * effective_size)
                )
                our_weight[0, 0, filter_set_index] = 1
            else:
                our_weight = torch.zeros(
                    (1, 1, 1, effective_size * effective_size)
                )
            ...
            output_np, output_np_max, index_max = forward_over_model(
                img_tensor, model_ses
            )
            (
                output_scaled_np,
                output_scaled_np_max,
                index_scaled_max,
            ) = forward_over_model(img_scaled_tensor, model_ses)
            if len(sesn_scales) > 2:
                (
                    output_scaled_np2,
                    output_scaled_np_max2,
                    index_scaled_max2,
                ) = forward_over_model(img_scaled_tensor2, model_ses)
            ...
            if use_global_vmax_vmin:
                ...
                if len(sesn_scales) > 2:
                    vmax = np.max(
                        [
                            np.max(output_np_max),
                            np.max(output_scaled_np_max),
                            np.max(output_scaled_np_max2),
                            np.max(img_single_ch_np),
                            np.max(img_scaled_single_ch_np),
                            np.max(img_scaled_single_ch_np2),
                        ]
                    )
                    vmin = np.min(
                        [
                            np.min(output_np_max),
                            np.min(output_scaled_np_max),
                            np.min(output_scaled_np_max2),
                            np.min(img_single_ch_np),
                            np.min(img_scaled_single_ch_np),
                            np.min(img_scaled_single_ch_np2),
                        ]
                    )
                else:
                    vmax = np.max(
            ...
            print(
                "Filter Index={} vmin= {:.2f} vmax= {:.2f}".format(
                    filter_set_index, vmin, vmax
                )
            )

            fig = plt.figure(
                figsize=(params.size[0] * 1.5, params.size[1] * 1.75),
                dpi=params.DPI,
            )
            ...
            if len(sesn_scales) > 2:
                ...
                # plt.axis('off')
                if (
                    len(index_scaled_max2) == 1
                    and index_scaled_max2.flatten() != 2
                ):
                    draw_red_border(sub2)

            if show_log_polar:
                output_np, output_np_max = forward_over_model(
                    img_tensor, model_logpolar, max_over_scale=False
                )
                output_scaled_np, output_scaled_np_max = forward_over_model(
                    img_scaled_tensor, model_logpolar, max_over_scale=False
                )
        ...
        basis_all = (
            model_ses[ses_layer_index].basis.cpu().numpy()
        )  # num_func x scale x h x w
        ...
        similarity = np.tensordot(
            basis_all_reshape, basis_all_reshape, axes=(1, 1)
        )
        ...
        similarity2 = np.tensordot(
            basis_all_reshape2, toy_image_all, axes=(1, 1)
        )

        # Get how many constraints satisfied
        basis_tensor = model_ses[ses_layer_index].basis
        num_satisfied_constraints = get_response_of_filter_bank(
            basis=basis_tensor, check_margin=0.01
        )
        print(
            "==> Number of constraints satisfied = {:3d}/{:3d}".format(
                num_satisfied_constraints, basis_tensor.shape[0] * 6
            )
        )
...
img_path = (
    "images/test_ses_equivariance_" + str(effective_size) + "_similarity.png"
)

...
from deviant.lib.helpers.dataloader_helper import build_dataloader
from deviant.lib.helpers.model_helper import build_model
from deviant.lib.helpers.optimizer_helper import build_optimizer
from deviant.lib.helpers.scheduler_helper import build_lr_scheduler
from deviant.lib.helpers.tester_helper import Tester
from deviant.lib.helpers.trainer_helper import Trainer
...
def create_logger(log_file):
    ...
    logging.basicConfig(
        level=logging.INFO, format=log_format, filename=log_file
    )
...
def init_torch(rng_seed, cuda_seed):
    """
    Initializes the seeds for ALL potential randomness, including torch,
        numpy, and random packages.

    Args:
        rng_seed (int): the shared random seed to use for numpy and random
        cuda_seed (int): the random seed to use for
            pytorch's torch.cuda.manual_seed_all function
    """
    # seed everything
...
def pretty_print(name, input, val_width=40, key_width=0):
    ...
    Args:
        ...
        key_width (int): the minimum key width, (always auto-defaults to the
            longest key!)

    Example:
        pretty_str = pretty_print('conf', conf.__dict__)
        pretty_str = pretty_print('conf', {'key1': 'example',
        'key2': [1,2,3,4,5], 'key3': np.random.rand(4,4)})
...
def main():
    ...
    exp_parent_dir = os.path.join(
        cfg["trainer"]["log_dir"], os.path.basename(args.config).split(".")[0]
    )

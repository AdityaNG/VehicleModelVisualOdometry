...
def get_sb_file_content(yaml_file_path, yaml_basename, time_in_hour, index=0):
    ...
    content += (
        "#SBATCH --time="
        + str(time_in_hour)
        + ":00:00             # limit of wall clock time - how long the job will run (same as -t)\n"
    )
    ...
    content += (
        "srun /mnt/home/kumarab6/anaconda3/envs/GUPNet_a/bin/python -u tools/train_val.py --config="
        + yaml_file_path
        + " ### Run python code\n"
    )
...
ap.add_argument(
    "-i",
    "--input",
    default="experiments/run_11.yaml",
    help="path of the yaml file",
)
ap.add_argument(
    "-t", "--time", type=int, default="10", help="running time in hours"
)
ap.add_argument(
    "-c",
    "--copies",
    type=int,
    default=3,
    help="number of copies of experiments",
)
args = ap.parse_args()
...
for i in range(1, total_copies):
    yaml_paths_list.append(
        os.path.join(yaml_folder, yaml_basename + "_" + str(i) + ".yaml")
    )
...
for i in range(total_copies):
    ...
    content = get_sb_file_content(
        yaml_file_path=yaml_paths_list[i],

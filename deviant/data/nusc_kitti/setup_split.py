...
def link_original_data(
    org_file_path, new_file_path, org_split_folders, new_split_folders
):
    # mkdirs
    ...
    for imind, line in enumerate(text_lines):
        ...
        if parsed is not None:
            ...
            org_calib_path = os.path.join(
                org_split_folders["cal"], id + ".txt"
            )
            org_label_path = os.path.join(
                org_split_folders["lab"], id + ".txt"
            )
            org_image_path = os.path.join(
                org_split_folders["ims"], id + ".png"
            )

            # If any of the calib/label/image is missing
            if (
                not os.path.exists(org_calib_path)
                or not os.path.exists(org_label_path)
                or not os.path.exists(org_image_path)
            ):
                print("{} not found ...".format(parsed))
                imind += 1
                continue

            new_calib_path = os.path.join(
                new_split_folders["cal"], str(new_id) + ".txt"
            )
            new_label_path = os.path.join(
                new_split_folders["lab"], str(new_id) + ".txt"
            )
            new_image_path = os.path.join(
                new_split_folders["ims"], str(new_id) + ".png"
            )
...
org_train_folders["cal"] = os.path.join(
    curr_folder, nusc_kitti_org, "train", "calib"
)
org_train_folders["ims"] = os.path.join(
    curr_folder, nusc_kitti_org, "train", "image_2"
)
org_train_folders["lab"] = os.path.join(
    curr_folder, nusc_kitti_org, "train", "label_2"
)
...
org_val_folders["cal"] = os.path.join(
    curr_folder, nusc_kitti_org, "val", "calib"
)
org_val_folders["ims"] = os.path.join(
    curr_folder, nusc_kitti_org, "val", "image_2"
)
org_val_folders["lab"] = os.path.join(
    curr_folder, nusc_kitti_org, "val", "label_2"
)
...
print("=============== Linking train =======================")
link_original_data(
    org_file_path=org_train_file,
    new_file_path=new_train_file,
    org_split_folders=org_train_folders,
    new_split_folders=new_train_folders,
)
...
print("=============== Linking val =======================")
link_original_data(
    org_file_path=org_val_file,
    new_file_path=new_val_file,
    org_split_folders=org_val_folders,
    new_split_folders=new_val_folders,
)

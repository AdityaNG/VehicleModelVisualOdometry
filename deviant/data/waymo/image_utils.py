...
def compute_orientation_3d(dim, location, rotation_y):
    ...
    orientation_3d = orientation_3d + np.array(
...
def draw_box_2d(image, corners, c=green_color):
    assert len(corners) == 4
    left = np.array(corners[0], dtype=np.float32)
    top = np.array(corners[1], dtype=np.float32)
    right = np.array(corners[2], dtype=np.float32)
    bottom = np.array(corners[3], dtype=np.float32)
    cv2.line(image, (left, top), (right, top), c, 1, lineType=cv2.LINE_AA)
    cv2.line(image, (left, top), (left, bottom), c, 1, lineType=cv2.LINE_AA)
    cv2.line(image, (right, top), (right, bottom), c, 1, lineType=cv2.LINE_AA)
    cv2.line(
        image, (left, bottom), (right, bottom), c, 1, lineType=cv2.LINE_AA
    )
    return image


def draw_box_centers(image, center1, center2=None):
    # print(center1, center2)
    cv2.circle(image, center1, 1, red_color, 4)
    if center2:
        cv2.circle(image, center2, 1, green_color, 4)
    return image

    ...
    try:
        if ignore_warnings:
            with warnings.catch_warnings():
                ...
                if use_pandas:
                    data = pd.read_csv(
                        path, delimiter=delimiter, header=None
                    ).values
                else:
        ...
        with open(file, "rb") as rf:
            ...
            for i in range(len(stuff1.objects)):
                ...
                if (
                    ...
                    and obj.num_lidar_points_in_box <= 5
                ):
                    difficulty.append(2)
                else:
                    difficulty.append(1)
    ...
    def get_boxes_from_txt(self, pd_set, gt_set, pd_dir, gt_dir):
        ...
        for i in range(len(pred_lines)):  # 39848
            ...
            gt_file_name = os.path.join(
                gt_dir, "validation_org", gt_seg, "label_0", gt_id + ".txt"
            )

            if not os.path.exists(pred_file) or not os.path.exists(
                gt_file_name
            ):
                continue
        ...
        return (
            np.array(pd_bbox, dtype=np.float32),
        ...
        with self.test_session(graph=graph) as sess:
            ...
            for category in category_list:
                for level in level_list:
                    ...
                    key_list = [
                        "OBJECT_TYPE_TYPE_{}_LEVEL_{}".format(category, level),
                        "RANGE_TYPE_{}_[0, 30)_LEVEL_{}".format(
                            category, level
                        ),
                        "RANGE_TYPE_{}_[30, 50)_LEVEL_{}".format(
                            category, level
                        ),
                        "RANGE_TYPE_{}_[50, +inf)_LEVEL_{}".format(
                            category, level
                        ),
                    ]
                    for metric in metric_list:
                        ...
                        multiplier = 100.0
                        text += "| {:5.2f} {:5.2f} {:5.2f} {:5.2f} ".format(
                            multiplier * aps[key0][0],
